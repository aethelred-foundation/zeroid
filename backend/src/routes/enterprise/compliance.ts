...
router.post('/report', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(201).json({ data: report, message: `${parsed.data} report generated` });
  } catch (err) {
    const error = err as Error & { statusCode?: number; code?: string };
...
router.post('/cross-border', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (jurisdictionAssessment.success) {
      ...
      res.status(200).json({ data: result, message: 'Cross-border assessment completed' });
      return;
    }
    ...
    if (transferAssessment.success) {
      ...
      res.status(200).json({ data: result, message: 'Data transfer assessment completed' });
      return;
    }
...
router.post('/dsar', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (requestType === 'erasure' || req.body.reportType === 'ERASURE') {
      ...
      res.status(200).json({ data: report, message: 'Erasure request processed' });
      return;
    }
