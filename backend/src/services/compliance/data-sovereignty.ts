...
export const DataResidencyRuleSchema = z.object({
  ...
  dataCategory: z.enum(['personal', 'financial', 'biometric', 'health', 'criminal', 'credential', 'metadata']),
  storageRegion: z.string(),
  encryptionRequired: z.boolean(),
  encryptionStandard: z.enum(['AES-256-GCM', 'AES-256-CBC', 'ChaCha20-Poly1305']).default('AES-256-GCM'),
...
export const CrossBorderTransferSchema = z.object({
  ...
  dataCategories: z.array(z.enum(['personal', 'financial', 'biometric', 'health', 'criminal', 'credential', 'metadata'])),
  dataSubjectId: z.string(),
  purpose: z.string(),
  legalBasis: z.enum([
    'adequacy_decision', 'standard_contractual_clauses', 'binding_corporate_rules',
    'explicit_consent', 'vital_interests', 'public_interest', 'legal_claims',
  ]).optional(),
...
export const ConsentRecordSchema = z.object({
  ...
  collectionMethod: z.enum(['explicit_form', 'api', 'in_app', 'verbal', 'written']),
  jurisdiction: z.string(),
...
export const PIASchema = z.object({
  ...
  dataSubjectCategories: z.array(z.enum(['employees', 'customers', 'minors', 'vulnerable_persons', 'general_public'])),
...
export interface PIAResult {
  ...
  findings: Array<{ area: string; risk: string; severity: string; mitigation: string }>;
  dpaRequired: boolean;
...
const DEFAULT_RESIDENCY_RULES: DataResidencyRule[] = [
  { jurisdiction: 'AE-CBUAE', dataCategory: 'personal', storageRegion: 'me-central-1', encryptionRequired: true, encryptionStandard: 'AES-256-GCM', localKeyManagement: true, replicationAllowed: false, replicationRegions: [] },
  { jurisdiction: 'AE-CBUAE', dataCategory: 'financial', storageRegion: 'me-central-1', encryptionRequired: true, encryptionStandard: 'AES-256-GCM', localKeyManagement: true, replicationAllowed: false, replicationRegions: [] },
  { jurisdiction: 'AE-CBUAE', dataCategory: 'biometric', storageRegion: 'me-central-1', encryptionRequired: true, encryptionStandard: 'AES-256-GCM', localKeyManagement: true, replicationAllowed: false, replicationRegions: [] },
  { jurisdiction: 'EU-GDPR', dataCategory: 'personal', storageRegion: 'eu-west-1', encryptionRequired: true, encryptionStandard: 'AES-256-GCM', localKeyManagement: false, replicationAllowed: true, replicationRegions: ['eu-central-1', 'eu-north-1'] },
  { jurisdiction: 'EU-GDPR', dataCategory: 'biometric', storageRegion: 'eu-west-1', encryptionRequired: true, encryptionStandard: 'AES-256-GCM', localKeyManagement: true, replicationAllowed: false, replicationRegions: [] },
  { jurisdiction: 'SA-SAMA', dataCategory: 'personal', storageRegion: 'me-south-1', encryptionRequired: true, encryptionStandard: 'AES-256-GCM', localKeyManagement: true, replicationAllowed: false, replicationRegions: [] },
  { jurisdiction: 'SA-SAMA', dataCategory: 'financial', storageRegion: 'me-south-1', encryptionRequired: true, encryptionStandard: 'AES-256-GCM', localKeyManagement: true, replicationAllowed: false, replicationRegions: [] },
  { jurisdiction: 'SG-MAS', dataCategory: 'personal', storageRegion: 'ap-southeast-1', encryptionRequired: true, encryptionStandard: 'AES-256-GCM', localKeyManagement: false, replicationAllowed: true, replicationRegions: ['ap-southeast-2'] },
  { jurisdiction: 'UK-FCA', dataCategory: 'personal', storageRegion: 'eu-west-2', encryptionRequired: true, encryptionStandard: 'AES-256-GCM', localKeyManagement: false, replicationAllowed: true, replicationRegions: ['eu-west-1'] },
  { jurisdiction: 'BH-CBB', dataCategory: 'personal', storageRegion: 'me-central-1', encryptionRequired: true, encryptionStandard: 'AES-256-GCM', localKeyManagement: true, replicationAllowed: false, replicationRegions: [] },
];
...
export class DataSovereigntyService {
  ...
  private transferAssessments: Map<string, TransferAssessmentResult> = new Map();
  ...
  // -------------------------------------------------------------------------
  getResidencyRules(jurisdiction: string, dataCategory?: string): DataResidencyRule[] {
    return this.residencyRules.filter(
      (r) => r.jurisdiction === jurisdiction && (!dataCategory || r.dataCategory === dataCategory),
    );
  }

  enforceResidency(jurisdiction: string, dataCategory: string, targetRegion: string): {
    compliant: boolean;
    ...
    const replicaMatch = rule.replicationAllowed && rule.replicationRegions.includes(targetRegion);

    if (!primaryMatch && !replicaMatch) {
      violations.push(`Data must be stored in ${rule.storageRegion}, not ${targetRegion}`);
    }
  ...
  withdrawConsent(dataSubjectId: string, purposeId: string): { withdrawn: boolean; affectedRecords: number } {
    ...
    for (const record of records) {
      if (record.withdrawable) {
        const purposeIndex = record.purposes.findIndex((p) => p.purposeId === purposeId);
        if (purposeIndex >= 0) {
  ...
  // -------------------------------------------------------------------------
  enforceMinimization(requestedFields: string[], purpose: string, jurisdiction: string): {
    allowedFields: string[];
    deniedFields: string[];
    reason: string;
  } {
    const minimizationRules: Record<string, string[]> = {
      identity_verification: ['full_name', 'date_of_birth', 'nationality', 'document_number', 'document_type', 'photo'],
      age_verification: ['date_of_birth'],
      address_verification: ['full_name', 'address_line1', 'address_line2', 'city', 'postal_code', 'country'],
      accreditation_check: ['full_name', 'accreditation_status', 'accreditation_date'],
    ...
    logger.info('minimization_enforced', { purpose, jurisdiction, requested: requestedFields.length, allowed: allowedFields.length, denied: deniedFields.length });

    return {
      allowedFields,
      deniedFields,
      reason: deniedFields.length > 0 ? `Fields not necessary for purpose "${purpose}" under data minimization principle` : 'All fields permitted for stated purpose',
    };
  }
  ...
  conductPIA(pia: PIA): PIAResult {
    ...
    const sensitiveCategories = parsed.dataCategories.filter((c) => ['biometric', 'health', 'criminal'].includes(c));
    if (sensitiveCategories.length > 0) {
      ...
      findings.push({
        ...
        severity: 'high',
        mitigation: 'Ensure GDPR Art. 9 lawful basis. Implement additional encryption and access controls.',
      });
    }

    // Vulnerable data subjects
    if (parsed.dataSubjectCategories.includes('minors') || parsed.dataSubjectCategories.includes('vulnerable_persons')) {
      riskScore += 25;
      findings.push({
        area: 'Vulnerable Data Subjects',
        risk: 'Processing data of minors or vulnerable persons',
        severity: 'high',
        mitigation: 'Implement age verification. Obtain parental consent where applicable.',
      });
    }
    ...
    if (parsed.automaticDecisionMaking) {
      ...
      findings.push({
        ...
        severity: 'medium',
        mitigation: 'Provide opt-out mechanism. Implement human review process.',
      });
    }
    ...
    const processorsWithoutDPA = parsed.thirdPartyProcessors.filter((p) => !p.dpaInPlace);
    if (processorsWithoutDPA.length > 0) {
      ...
      findings.push({
        ...
        severity: 'high',
        mitigation: 'Execute DPAs with all processors before data sharing. Ensure GDPR Art. 28 compliance.',
      });
    }
    ...
    const riskLevel: PIAResult['riskLevel'] = riskScore >= 75 ? 'very_high' : riskScore >= 50 ? 'high' : riskScore >= 25 ? 'medium' : 'low';
    const dpiaRequired = riskScore >= 50 || sensitiveCategories.length > 0 || parsed.automaticDecisionMaking;
    const supervisoryConsultationRequired = riskScore >= 75;

    const recommendations: string[] = [];
    if (dpiaRequired) recommendations.push('Conduct full DPIA before proceeding with processing');
    if (processorsWithoutDPA.length > 0) recommendations.push('Execute DPAs with all data processors');
    if (parsed.crossBorderTransfer) recommendations.push('Complete Transfer Impact Assessment');
    if (findings.length === 0) recommendations.push('No significant privacy risks identified');
  ...
  initiateBreachNotification(breach: BreachNotification): BreachTimeline {
    ...
    const regulatoryDeadlines = parsed.jurisdictions.map((j) => {
      ...
      return {
        ...
        deadline: new Date(detectedTime + rule.hours * 60 * 60 * 1000).toISOString(),
        notificationSent: false,
        sentAt: null,
      };
    });

    const subjectNotificationRequired = parsed.severity === 'critical' || parsed.severity === 'high';
  ...
  trackRetention(dataSubjectId: string, category: string, jurisdiction: string, retentionDays: number): void {
    ...
    const status = this.retentionTracker.get(dataSubjectId) ?? { dataSubjectId, records: [] };
    status.records.push({
      category,
      jurisdiction,
      retentionDays,
    ...
    for (const [subjectId, status] of this.retentionTracker) {
      for (const record of status.records) {
        if (new Date(record.expiresAt).getTime() <= now && !record.expired) {
          expired.push({ dataSubjectId: subjectId, category: record.category, expiresAt: record.expiresAt });
        }
      }
    }
