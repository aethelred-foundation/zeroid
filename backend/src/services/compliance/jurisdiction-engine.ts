...
export type ComplianceEvaluationRequest = z.infer<
...
export const CrossBorderAssessmentSchema = z.object({
  ...
  dataCategories: z.array(
    z.enum(["personal", "financial", "biometric", "health", "criminal"]),
  ),
  purpose: z.string(),
});
...
const MUTUAL_RECOGNITION_AGREEMENTS: MutualRecognition[] = [
  {
    jurisdictions: ["AE-ADGM", "UK-FCA"],
    credentialTypes: ["kyc_enhanced", "accredited_investor"],
    effectiveDate: "2024-01-01",
    conditions: ["credential_age_lt_180d"],
  },
  {
    jurisdictions: ["AE-DIFC", "SG-MAS"],
    credentialTypes: ["kyc_enhanced", "corporate_verification"],
    effectiveDate: "2024-06-01",
    conditions: ["credential_age_lt_365d"],
  },
  {
    jurisdictions: ["EU-EIDAS", "UK-FCA"],
    credentialTypes: ["national_id", "kyc_basic", "kyc_enhanced"],
    effectiveDate: "2024-03-01",
    conditions: ["post_brexit_agreement"],
  },
  {
    jurisdictions: ["SG-MAS", "BH-CBB"],
    credentialTypes: ["kyc_basic", "aml_clearance"],
    effectiveDate: "2024-09-01",
    conditions: [],
  },
  {
    jurisdictions: ["AE-CBUAE", "BH-CBB"],
    credentialTypes: [
      "kyc_basic",
      "kyc_enhanced",
      "aml_clearance",
    ...
    conditions: ["gcc_cooperation"],
  },
  {
    jurisdictions: ["AE-CBUAE", "SA-SAMA"],
    credentialTypes: ["kyc_basic", "kyc_enhanced", "aml_clearance"],
    effectiveDate: "2023-06-01",
    conditions: ["gcc_cooperation"],
  },
  {
    jurisdictions: ["EU-EIDAS", "EU-MICA"],
    credentialTypes: [
      "national_id",
      "kyc_basic",
      "kyc_enhanced",
...
// ---------------------------------------------------------------------------
const JURISDICTION_REQUIREMENTS: Map<
  JurisdictionCode,
  Map<string, string[]>
> = new Map([
  [
    ...
    new Map([
      [
        "onboarding",
        ["emirates_id", "passport", "proof_of_address", "source_of_funds"],
      ],
      ["transaction", ["kyc_basic", "aml_clearance"]],
      ["transfer", ["kyc_enhanced", "sanctions_clearance", "source_of_funds"]],
      [
        "periodic_review",
        ["kyc_enhanced", "aml_clearance", "proof_of_address"],
      ],
    ]),
  ],
  [
  ...
  [
    ...
    new Map([
      [
        "onboarding",
        ["passport", "proof_of_address", "professional_reference"],
      ],
      ["transaction", ["kyc_enhanced", "aml_clearance"]],
      ["transfer", ["kyc_enhanced", "sanctions_clearance"]],
      ["periodic_review", ["kyc_enhanced", "aml_clearance"]],
  ...
  [
    ...
    new Map([
      [
        "onboarding",
        ["national_id", "proof_of_address", "eidas_qualified_cert"],
      ],
      ["transaction", ["kyc_basic"]],
  ...
  [
    ...
    new Map([
      ...
      [
        "transfer",
        ["kyc_enhanced", "sanctions_clearance", "travel_rule_compliance"],
      ],
      ["periodic_review", ["kyc_enhanced", "aml_clearance"]],
    ]),
  ],
  [
    ...
    new Map([
      [
        "onboarding",
        ["government_id", "accredited_investor", "ssn_verification"],
      ],
  ...
  [
    ...
    new Map([
      [
        "onboarding",
        ["government_id", "ssn_verification", "bitlicense_clearance"],
      ],
  ...
  [
    ...
    new Map([
      [
        "onboarding",
        ["nric_or_passport", "proof_of_address", "source_of_wealth"],
      ],
      ["transaction", ["kyc_enhanced"]],
      [
        "transfer",
  ...
  [
    ...
    new Map([
      [
        "onboarding",
        ["national_id_absher", "proof_of_address", "source_of_funds"],
      ],
      ["transaction", ["kyc_enhanced"]],
      ["transfer", ["kyc_enhanced", "sanctions_clearance", "source_of_funds"]],
...
export class JurisdictionEngine extends EventEmitter {
  ...
  constructor() {
    super();
    logger.info("JurisdictionEngine initialized", {
      jurisdictions: JURISDICTION_REGISTRY.size,
    });
  }
  ...
  detectJurisdictions(claims: Record<string, unknown>): JurisdictionCode[] {
    ...
    const country = (claims.country ??
      claims.nationality ??
      claims.residence_country ??
      "") as string;
    ...
    if (idType.includes("eidas")) {
      detected.add("EU-EIDAS");
      detected.add("EU-GDPR");
    }
    ...
    logger.debug("jurisdiction_detection", {
      claims_keys: Object.keys(claims),
      detected: [...detected],
    });
    return [...detected];
  }
  ...
  private async evaluateJurisdiction(
    ...
    const presentedTypes = new Set(
      request.credentials.map((c) => c.credentialType),
    );
    const missingCredentials = requiredCreds.filter(
      (r) => !presentedTypes.has(r),
    );
    const now = new Date();
    ...
    if (missingCredentials.length > 0 || hasCriticalFailure) {
      overallStatus =
        missingCredentials.length === requiredCreds.length
          ? "non_compliant"
          : "partial";
    } else if (hasWarning) {
  ...
  private evaluateRules(
    ...
    rules.push({
      ...
      detail:
        missingCreds.length === 0
          ? "All required KYC credentials present"
    ...
    rules.push({
      ...
      detail:
        staleCredentials.length === 0
          ? "All credentials within validity period"
          : `${staleCredentials.length} credential(s) older than 1 year`,
    });
    ...
    const hasRecognition = crossJurisdictionCreds.every((c) =>
      this.checkMutualRecognition(
        c.issuingJurisdiction!,
        jurisdiction,
        c.credentialType,
      ),
    );
    rules.push({
      ...
      status:
        crossJurisdictionCreds.length === 0 || hasRecognition
          ? "pass"
          : "warning",
      detail:
        crossJurisdictionCreds.length === 0
          ? "All credentials from target jurisdiction"
          : hasRecognition
            ? "Cross-jurisdiction credentials covered by MRA"
            : "Some credentials from unrecognized jurisdictions",
    });
    ...
    if (meta.consentModel === "explicit") {
      const hasConsent = request.credentials.some(
        (c) =>
          c.credentialType.includes("consent") ||
          c.claims.consent_given === true,
      );
      rules.push({
        ruleId: crypto.randomUUID(),
        ...
        detail: hasConsent
          ? "Explicit consent credential present"
  ...
  assessCrossBorder(assessment: CrossBorderAssessment): CrossBorderResult {
    ...
    const mra = this.findMutualRecognition(
      sourceJurisdiction,
      targetJurisdiction,
    );
    ...
    const sourceReqs =
      JURISDICTION_REQUIREMENTS.get(sourceJurisdiction)?.get("transfer") ?? [];
    const targetReqs =
      JURISDICTION_REQUIREMENTS.get(targetJurisdiction)?.get("onboarding") ??
      [];
    const additionalRequired = targetReqs.filter(
      (r) => !sourceReqs.includes(r) && !acceptedCredentials.includes(r),
    );

    let dataTransferMechanism: CrossBorderResult["dataTransferMechanism"] =
      "not_required";
    ...
    if (sourceMeta.region === "europe" || targetMeta.region === "europe") {
      if (sourceMeta.region === targetMeta.region) {
        ...
        restrictions.push(
          "EU SCCs required for cross-border personal data transfer",
        );
      }
    }
    ...
    if (dataCategories.includes("biometric")) {
      restrictions.push(
        "Biometric data transfer requires explicit consent in both jurisdictions",
      );
    }
    ...
    if (targetMeta.crossBorderRestricted) {
      restrictions.push(
        `${targetMeta.name} imposes additional cross-border restrictions`,
      );
    }

    const allowed =
      restrictions.length < 3 &&
      (mra !== null || !targetMeta.crossBorderRestricted);
  ...
  getRequirementDiff(
    ...
    const newReqs =
      JURISDICTION_REQUIREMENTS.get(newJurisdiction)?.get(operationType) ?? [];
  ...
  publishRegulatoryChange(
    notification: Omit<RegulatoryChangeNotification, "id" | "publishedAt">,
    ...
    logger.info("regulatory_change_published", {
      id: change.id,
  ...
  // Internal helpers
  // -------------------------------------------------------------------------
  private checkMutualRecognition(
  ...
  private findMutualRecognition(
    a: JurisdictionCode,
    b: JurisdictionCode,
  ): MutualRecognition | null {
    return (
      MUTUAL_RECOGNITION_AGREEMENTS.find(
        (mra) =>
