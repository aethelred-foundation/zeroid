...
export class AgentIdentityService {
  ...
  async createDelegation(
    ...
    const delegatorCapNames = new Set(
      fromAgent.capabilities.map((c) => c.name),
    );
  ...
  async verifyAgent(
    ...
    if (!signatureValid) {
      ...
      await this.recordAnomalyEvent(
        agent,
        "Invalid signature presented for verification",
      );
    ...
    for (const requested of request.requestedCapabilities) {
      if (allAvailableCaps.has(requested)) {
        ...
        denied.push({
          name: requested,
          reason: "Capability not granted to this agent",
        });
      }
    }
    ...
    agent.stats.averageLatencyMs =
      (agent.stats.averageLatencyMs * (agent.stats.totalActions - 1) +
        latencyMs) /
      agent.stats.totalActions;
    this.agents.set(agent.agentId, agent);

    details.push(
      `Authorized: ${authorized.length}/${request.requestedCapabilities.length} capabilities`,
    );
  ...
  private async detectVerificationAnomaly(
    ...
    if (baseline && baseline.sampleCount >= 10) {
      ...
      if (unusualCaps.length > 0) {
        anomalies.push(
          `Unusual capabilities requested: ${unusualCaps.join(", ")}`,
        );
      }
  ...
  private async recordAnomalyEvent(
    ...
    if (agent.stats.anomalyCount >= 10 && agent.status === "active") {
      ...
      await this.suspendAgent(
        ...
        `Automatic suspension: ${agent.stats.anomalyCount} anomalies detected`,
      );
    }
  }
  ...
  private async createApprovalRequest(
    ...
    const request: HumanApprovalRequest = {
      ...
      resourceId,
      riskLevel:
        agent.capabilities.find((c) => c.name === action)?.riskLevel ?? "high",
      context: {
        purpose,
        agentName: agent.agentName,
        protocol: agent.agentProtocol,
  ...
  private async getDelegatedCapabilities(
    agentId: string,
  ): Promise<Set<string>> {
    const delegationIds = await redis.smembers(`delegations:to:${agentId}`);
  ...
  private async traceDelegationChain(
    fromAgentId: string,
    toAgentId: string,
  ): Promise<string[]> {
    ...
    for (let depth = 0; depth < 10; depth++) {
      const delegationIds = await redis.smembers(
        `delegations:from:${currentId}`,
      );
      let found = false;
      ...
      for (const delId of delegationIds) {
        ...
        if (
          delegation &&
          delegation.status === "active" &&
          !visited.has(delegation.toAgentId)
        ) {
