...
describe("AuditTimeline", () => {
  ...
  it("renders event descriptions", () => {
    ...
    expect(
      screen.getByText("ZK proof generated for age verification"),
    ).toBeInTheDocument();
  });
  ...
  it("opens filter dropdown and shows event type options", () => {
    ...
    expect(
      screen.getAllByText("Credential Issued").length,
    ).toBeGreaterThanOrEqual(1);
    expect(
      screen.getAllByText("Credential Revoked").length,
    ).toBeGreaterThanOrEqual(1);
  });
  ...
  it("filters events by credential-revoked type", () => {
    ...
    expect(
      screen.queryByText("ZK proof generated for age verification"),
  ...
  it("resets filter back to All Events", () => {
    ...
    const filterButton = screen
      .getAllByText("Credential Issued")[0]
      .closest("button")!;
  ...
  it("closes filter dropdown after selecting a filter", () => {
    ...
    expect(
      screen.getAllByText("Credential Verified").length,
    ).toBeGreaterThanOrEqual(1);
  ...
  it("renders events of all supported types", () => {
    const allTypeEvents = [
      ...
      {
        id: "e7",
        type: "selective-disclosure" as const,
        timestamp: "2026-03-15T10:00:00Z",
        description: "test7",
      },
    ];
  ...
  it("shows all filter options in dropdown", () => {
    ...
    expect(
      screen.getAllByText("Proof Generated").length,
    ).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText("Proof Verified").length).toBeGreaterThanOrEqual(
      1,
    );
    expect(
      screen.getAllByText("Identity Created").length,
    ).toBeGreaterThanOrEqual(1);
    expect(
      screen.getAllByText("Selective Disclosure").length,
    ).toBeGreaterThanOrEqual(1);
  });
