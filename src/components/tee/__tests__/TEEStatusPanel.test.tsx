...
describe('TEEStatusPanel', () => {
  ...
  it('renders loading state', () => {
    mockUseTEE.mockReturnValue({ nodes: [], attestation: null, isLoading: true, error: null, refreshStatus: jest.fn() });
    render(<TEEStatusPanel />);
    expect(screen.getByText('Loading TEE status...')).toBeInTheDocument();
  });

  it('renders error state', () => {
    mockUseTEE.mockReturnValue({ nodes: [], attestation: null, isLoading: false, error: 'fail', refreshStatus: jest.fn() });
    render(<TEEStatusPanel />);
  ...
  it('renders empty node state', () => {
    mockUseTEE.mockReturnValue({ nodes: [], attestation: null, isLoading: false, error: null, refreshStatus: jest.fn() });
    render(<TEEStatusPanel />);
    expect(screen.getByText('No TEE nodes available')).toBeInTheDocument();
  });

  it('handles null nodes gracefully in full mode', () => {
    mockUseTEE.mockReturnValue({ nodes: null, attestation: null, isLoading: false, error: null, refreshStatus: jest.fn() });
    render(<TEEStatusPanel />);
  ...
  it('shows all healthy indicator in compact mode when all nodes are healthy', () => {
    ...
    mockUseTEE.mockReturnValue({ nodes: allHealthyNodes, attestation: null, isLoading: false, error: null, refreshStatus: jest.fn() });
    render(<TEEStatusPanel compact={true} />);
  ...
  it('shows Never when attestation lastVerified is null', () => {
    ...
    mockUseTEE.mockReturnValue({ nodes: mockNodes, attestation: noVerifiedAttestation, isLoading: false, error: null, refreshStatus: mockRefreshStatus });
    render(<TEEStatusPanel />);
  ...
  it('does not render enclave ID when not provided', () => {
    ...
    mockUseTEE.mockReturnValue({ nodes: mockNodes, attestation: noEnclaveAttestation, isLoading: false, error: null, refreshStatus: mockRefreshStatus });
    render(<TEEStatusPanel />);
  ...
  it('renders unknown health status for nodes with unknown health', () => {
    const unknownNode = [
      { id: 'n1', name: 'Unknown Node', region: 'us-east-1', health: 'unknown' },
    ];
    mockUseTEE.mockReturnValue({ nodes: unknownNode, attestation: null, isLoading: false, error: null, refreshStatus: jest.fn() });
    render(<TEEStatusPanel />);
  ...
  it('handles refresh completing successfully', async () => {
    ...
    mockUseTEE.mockReturnValue({ nodes: mockNodes, attestation: mockAttestation, isLoading: false, error: null, refreshStatus: successRefresh });
    render(<TEEStatusPanel />);
    fireEvent.click(screen.getByText('Refresh'));
    await waitFor(() => {
