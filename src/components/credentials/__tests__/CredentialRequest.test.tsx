...
describe('CredentialRequest', () => {
  ...
  it('renders the initial select step', () => {
    ...
    expect(screen.getByText('Submit documents for TEE-verified credential issuance')).toBeInTheDocument();
  ...
  it('shows error when continue clicked without selecting schema', () => {
    ...
    expect(screen.getByText('Please select a credential type')).toBeInTheDocument();
  });
  ...
  it('proceeds to documents step after selecting schema and clicking continue', () => {
    ...
    fireEvent.click(screen.getByText('Identity Credential'));
    fireEvent.click(screen.getByText('Continue'));
    expect(screen.getByText(/Upload the required documents for/)).toBeInTheDocument();
  });
  ...
  it('handles file input with no file selected', () => {
    ...
    expect(screen.queryByText('File size must be under 10MB')).not.toBeInTheDocument();
