...
jest.mock('framer-motion', () => ({
  motion: {
    ...
    g: ({ children, onClick, ...props }: any) => <g onClick={onClick}>{children}</g>,
...
describe('AgentDelegationGraph', () => {
  ...
  it('calls onNodeClick when a node is clicked', () => {
    ...
    if (nodeNames[0]?.closest('g')) {
      ...
      expect(onNodeClick).toHaveBeenCalledWith(expect.objectContaining({ id: 'root' }));
    }
  });
  ...
  it('shows detail panel when node is selected', () => {
    ...
    if (nodeName[0]?.closest('g')) {
      ...
      expect(screen.getByText('did:aethelred:mainnet:0xdef456')).toBeInTheDocument();
  ...
  it('applies custom className', () => {
    const { container } = render(
      <AgentDelegationGraph nodes={mockNodes} edges={mockEdges} className="custom-class" />
    );
    expect(container.firstChild).toHaveClass('custom-class');
  });

  it('handles mouse down/move/up for panning', () => {
    const { container } = render(<AgentDelegationGraph nodes={mockNodes} edges={mockEdges} />);
    const panArea = container.querySelector('.relative.h-\\[450px\\]') as HTMLElement;
  ...
  it('renders edges with empty capabilities when not highlighted', () => {
    ...
    render(<AgentDelegationGraph nodes={mockNodes} edges={edgesNoCapabilities} />);
    expect(screen.getByText('Agent Delegation Graph')).toBeInTheDocument();
  });
  ...
  it('does not show detail panel when selecting a nonexistent node ID (covers ?? null fallback)', () => {
    ...
    const { rerender } = render(<AgentDelegationGraph nodes={mockNodes} edges={mockEdges} />);
    // Select agent-1
    const nodeName = screen.getAllByText('Compliance Engine');
    if (nodeName[0]?.closest('g')) {
      fireEvent.click(nodeName[0].closest('g')!);
    ...
    expect(screen.queryByText('did:aethelred:mainnet:0xdef456')).not.toBeInTheDocument();
  });
