...
describe('ComplianceCopilot', () => {
  ...
  it('renders the empty state with conversation starters', () => {
    ...
    expect(screen.getAllByText('Compliance Copilot').length).toBeGreaterThanOrEqual(1);
    expect(screen.getByText('AI-powered regulatory assistant')).toBeInTheDocument();
  ...
  it('renders input field and send button', () => {
    render(<ComplianceCopilot />);
    expect(screen.getByPlaceholderText('Ask about compliance, regulations, or risk...')).toBeInTheDocument();
  ...
  it('sends a message when user types and clicks send', () => {
    render(<ComplianceCopilot />);
    const input = screen.getByPlaceholderText('Ask about compliance, regulations, or risk...');
    fireEvent.change(input, { target: { value: 'What is compliance status?' } });
    const sendButton = screen.getByLabelText('Send message');
  ...
  it('clears input after sending a message', () => {
    render(<ComplianceCopilot />);
    const input = screen.getByPlaceholderText('Ask about compliance, regulations, or risk...') as HTMLInputElement;
    fireEvent.change(input, { target: { value: 'Test message' } });
  ...
  it('sends message when clicking a conversation starter', () => {
    ...
    expect(screen.getByText('Run a KYC screening on the latest onboarded identity')).toBeInTheDocument();
  });
  ...
  it('calls onAction when action button is clicked', async () => {
    ...
    const actionButtons = screen.queryAllByRole('button').filter(
      (btn) => btn.textContent && ['Run Screening', 'View Details', 'Generate Full Report', 'View Last Report'].includes(btn.textContent.trim())
    );
  ...
  it('shows suggested prompts when conversation has few messages', async () => {
    ...
    const starterButtons = allButtons.filter(
      (btn) => btn.textContent && ['Run KYC screening', 'Compliance status', 'Regulatory updates'].includes(btn.textContent.trim())
    );
  ...
  it('uses starter prompt to click on conversation starters from suggested prompts area', async () => {
    ...
    expect(screen.getByText('What is our current compliance status across all jurisdictions?')).toBeInTheDocument();
  ...
  it('handles action button click without onAction prop (covers onAction?. branch)', async () => {
    ...
    const actionButtons = screen.queryAllByRole('button').filter(
      (btn) => btn.textContent && ['Run Screening', 'View Details', 'View Last Report'].includes(btn.textContent.trim())
    );
    if (actionButtons.length > 0) {
  ...
  it('clicks suggested prompt during active conversation', async () => {
    ...
    const suggestedButtons = screen.getAllByRole('button').filter(
      (btn) => btn.textContent === 'Run KYC screening'
    );
