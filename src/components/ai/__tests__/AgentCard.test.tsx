...
describe('AgentCard', () => {
  ...
  it('renders delegation chain when delegation is provided', () => {
    ...
    expect(screen.getByText('did:aethelred:mainnet:0xdelegator')).toBeInTheDocument();
  ...
  it('applies className to loading state', () => {
    const { container } = render(<AgentCard agent={mockAgent} loading className="load-class" />);
    expect(container.innerHTML).toContain('load-class');
  });

  it('applies className to compact state', () => {
    const { container } = render(<AgentCard agent={mockAgent} compact className="compact-class" />);
