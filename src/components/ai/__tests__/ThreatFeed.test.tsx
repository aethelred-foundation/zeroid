...
describe("ThreatFeed", () => {
  ...
  it("displays event titles", () => {
    ...
    expect(
      screen.getAllByText("Identity Compromise").length,
    ).toBeGreaterThanOrEqual(1);
    expect(
      screen.getAllByText("Credential Fraud").length,
    ).toBeGreaterThanOrEqual(1);
  });
  ...
  it("opens filter panel and filters by severity", () => {
    ...
    const filterButton = buttons.find((b) =>
      b.querySelector('[data-testid="icon-filter"]'),
    );
  ...
  it("expands an event card to show details", () => {
    ...
    expect(
      screen.getByText("Full analysis available for this event"),
    ).toBeInTheDocument();
    expect(
  ...
  it("shows Reviewed label for reviewed events when expanded", () => {
    ...
    render(<ThreatFeed events={eventsWithDetails} autoRefresh={false} />);
    fireEvent.click(
      screen.getByText("Fraudulent credential presentation intercepted"),
    );
  ...
  it("calls onEventClick when expanding an event", () => {
    ...
    expect(onEventClick).toHaveBeenCalledWith(
      expect.objectContaining({ id: "threat-1" }),
    );
  });
  ...
  it("handles internal review when no onReview callback is provided", () => {
    ...
    const eventCards = allButtons.filter((btn) =>
  ...
  it("applies className to loading state", () => {
    const { container } = render(
      <ThreatFeed loading className="loading-class" autoRefresh={false} />,
    );
  ...
  it("applies className to error state", () => {
    const { container } = render(
      <ThreatFeed error="fail" className="error-class" autoRefresh={false} />,
    );
  ...
  it("filters events by severity and resets with All", () => {
    ...
    expect(
      screen.queryByText("Potential identity takeover detected"),
  ...
  it("sets type filter via type filter buttons and verifies active styling", () => {
    ...
    const typeFilterButtons = screen
      .getAllByRole("button")
      .filter(
        (btn) =>
          btn.className.includes("rounded-full") &&
          btn.textContent === "Sanctions Match",
      );
  ...
  it("shows all event types in type filters", () => {
    const allTypeEvents = [
      ...
      {
        id: "t5",
        type: "anomalous_behavior" as const,
        severity: "critical" as const,
        title: "T5",
        description: "D5",
        source: "S5",
