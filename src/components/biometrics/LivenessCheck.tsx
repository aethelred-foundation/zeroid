...
const LIVENESS_STEPS: { key: LivenessStep; label: string; instruction: string; icon: typeof Eye }[] = [
  { key: 'look_straight', label: 'Look Straight', instruction: 'Position your face in the center of the frame and look directly at the camera', icon: Eye },
  { key: 'turn_left', label: 'Turn Left', instruction: 'Slowly turn your head to the left', icon: ArrowLeft },
  { key: 'turn_right', label: 'Turn Right', instruction: 'Slowly turn your head to the right', icon: ArrowRight },
  { key: 'blink', label: 'Blink', instruction: 'Blink your eyes naturally two times', icon: Eye },
];
...
const ANTI_SPOOF_INDICATORS = [
  ...
  { name: 'Motion Consistency', description: 'Natural movement pattern verification' },
...
function FaceOverlay({
  ...
  const color =
    status === 'success' ? '#10b981' : status === 'failure' ? '#ef4444' : '#0ea5e9';
...
function ConfidenceMeter({ confidence }: { confidence: number }) {
  ...
  const label = confidence >= 90 ? 'High Confidence' : confidence >= 70 ? 'Moderate' : 'Low Confidence';

  return (
    <div className="p-3 rounded-xl bg-[var(--surface-secondary)]">
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-xs text-[var(--text-secondary)]">Liveness Confidence</span>
          ...
          <div key={step.key} className="flex items-center gap-2">
            ...
            >
              {isComplete ? (
                ...
                <span className="text-[10px] font-medium text-white/60">{idx + 1}</span>
              )}
            </motion.div>
            {idx < steps.length - 1 && (
              <div
                className={`w-8 h-0.5 rounded ${
                  idx < currentStepIdx ? 'bg-emerald-500' : 'bg-[var(--surface-tertiary)]'
                }`}
              />
            )}
          </div>
...
export default function LivenessCheck({
  ...
  const [status, setStatus] = useState<CheckStatus>(autoStart ? 'preparing' : 'idle');
  const [currentStepIdx, setCurrentStepIdx] = useState(0);
  const [progress, setProgress] = useState(0);
  const [confidence, setConfidence] = useState(0);
  const [antiSpoofResults, setAntiSpoofResults] = useState<Record<string, boolean>>({});
      ...
      setProgress((prev) => {
        ...
        if (next >= 100) {
          ...
          if (nextIdx >= LIVENESS_STEPS.length) {
            ...
            const success = finalConfidence >= 80 && Object.values(results).every(Boolean);
  ...
  if (loading) {
    return (
      <div className={`card p-8 flex items-center justify-center gap-2 ${className}`}>
        ...
        <span className="text-sm text-[var(--text-secondary)]">Initializing camera...</span>
      </div>
    );
  }
    ...
    <div className={`rounded-2xl border border-[var(--border-primary)] bg-[var(--surface-primary)] overflow-hidden ${className}`}>
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border-primary)]">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-[var(--text-primary)]">Liveness Verification</h3>
        </div>
      ...
      <div className="p-5 space-y-5">
        {/* Camera preview area */}
        <div className="relative rounded-2xl bg-zero-950 overflow-hidden" style={{ minHeight: 300 }}>
          ...
          <AnimatePresence mode="wait">
            {status === 'in_progress' && currentStep && (
              <motion.div
                ...
                exit={{ opacity: 0, y: 10 }}
              >
                <p className="text-sm font-medium text-white mb-1">{currentStep.label}</p>
                <p className="text-xs text-white/60">{currentStep.instruction}</p>
              </motion.div>
            )}
            {status === 'idle' && (
              <motion.div
            ...
            {status === 'preparing' && (
              ...
              >
                ...
                <p className="text-xs text-white/40 mt-1">Ensure good lighting and face the camera</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
        ...
        {(status === 'success' || status === 'failure') && Object.keys(antiSpoofResults).length > 0 && (
          <div>
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {ANTI_SPOOF_INDICATORS.map((indicator) => {
                ...
                return (
                  <div key={indicator.name} className="flex items-center gap-2 p-2.5 rounded-lg bg-[var(--surface-secondary)]">
                    {passed ? (
                    ...
                    <div>
                      <p className="text-[10px] font-medium text-[var(--text-primary)]">{indicator.name}</p>
                      <p className="text-[8px] text-[var(--text-tertiary)]">{indicator.description}</p>
                    </div>
                  </div>
                );
              })}
        ...
        <AnimatePresence>
          {status === 'success' && (
            ...
            >
              ...
              <p className="text-sm font-medium text-emerald-400">Liveness Verified</p>
              <p className="text-xs text-[var(--text-secondary)] mt-1">
                Your biometric liveness has been confirmed with {Math.round(confidence)}% confidence
              </p>
            </motion.div>
          )}
          {status === 'failure' && (
            ...
            >
              ...
              <p className="text-xs text-[var(--text-secondary)] mt-1">
                Unable to confirm liveness. Please try again with better lighting.
              </p>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="flex items-start gap-2 p-3 rounded-xl bg-blue-500/5 border border-blue-500/10">
          ...
          <div>
            <p className="text-[10px] font-medium text-blue-400">Privacy Notice</p>
            <p className="text-[10px] text-[var(--text-secondary)] mt-0.5">
              All biometric data is processed exclusively within a Trusted Execution Environment (TEE).
              Raw biometric data never leaves the enclave. Only a cryptographic hash is stored on-chain.
            </p>
          </div>
        </div>
      </div>
