...
const mockRoutes = [
  { from: 'US', to: 'EU', compliant: true, requirements: ['eIDAS 2.0 mapping'] },
];
...
describe('JurisdictionMap', () => {
  ...
  it('renders header', () => {
    render(<JurisdictionMap jurisdictions={mockJurisdictions} routes={mockRoutes} />);
  ...
  it('renders routes with unknown region IDs gracefully', () => {
    ...
    render(<JurisdictionMap jurisdictions={mockJurisdictions} routes={badRoutes} />);
    expect(screen.getByText('Jurisdiction Map')).toBeInTheDocument();
  });

  it('renders non-compliant routes with dashed style', () => {
    const nonCompliantRoutes = [
      { from: 'US', to: 'EU', compliant: false, requirements: ['missing requirement'] },
    ];
    render(<JurisdictionMap jurisdictions={mockJurisdictions} routes={nonCompliantRoutes} />);
    expect(screen.getByText('Jurisdiction Map')).toBeInTheDocument();
  });
  ...
  it('shows notes in sidebar when jurisdiction has notes', () => {
    ...
    render(<JurisdictionMap jurisdictions={jurisdictionsWithNotes} routes={mockRoutes} />);
    const usTexts = screen.getAllByText('US');
    const group = usTexts[0]?.closest('g');
    if (group) {
      fireEvent.click(group);
  ...
  it('renders jurisdiction score below 60 with red bar', () => {
    const lowScoreJurisdictions = [
      { ...mockJurisdictions[0], score: 45 },
    ];
    render(<JurisdictionMap jurisdictions={lowScoreJurisdictions} routes={[]} />);
    const usTexts = screen.getAllByText('US');
    const group = usTexts[0]?.closest('g');
    if (group) {
      fireEvent.click(group);
  ...
  it('renders jurisdiction score between 60-79 with amber bar', () => {
    const midScoreJurisdictions = [
      { ...mockJurisdictions[0], score: 65 },
    ];
    render(<JurisdictionMap jurisdictions={midScoreJurisdictions} routes={[]} />);
    const usTexts = screen.getAllByText('US');
    const group = usTexts[0]?.closest('g');
    if (group) {
      fireEvent.click(group);
