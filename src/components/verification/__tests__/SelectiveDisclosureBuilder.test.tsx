...
describe("SelectiveDisclosureBuilder", () => {
  ...
  it("renders empty state when no attributes available", () => {
    jest
      .spyOn(require("@/hooks/useCredentials"), "useCredentials")
      .mockReturnValue({
        credentials: [],
      });
    render(
  ...
  it("changes mode to zk-prove when ZK button is clicked", () => {
    ...
    expect(
      screen.getAllByText("Proved without revealing value").length,
  ...
  it("displays correct summary counts after mode changes", () => {
    ...
    const disclosedCounter = disclosedLabel
      .closest("div")
  ...
  it("shows mode description for each attribute", () => {
    ...
    expect(
      screen.getAllByText("Not included in proof").length,
    ).toBeGreaterThanOrEqual(1);
  });
