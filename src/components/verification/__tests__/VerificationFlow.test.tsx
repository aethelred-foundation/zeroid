...
jest.mock("@/hooks/useVerification", () => ({
  useVerification: () => ({
    submitProof: jest
      .fn()
      .mockResolvedValue({ verified: true, transactionHash: "0xtx123" }),
    isVerifying: false,
  }),
}));
...
describe("VerificationFlow", () => {
  ...
  it("renders default subtitle when no request", () => {
    ...
    expect(
      screen.getByText("Generate a zero-knowledge proof of your credentials"),
    ).toBeInTheDocument();
  });
  ...
  it("renders SelectiveDisclosureBuilder on first step", () => {
    render(<VerificationFlow request={mockRequest as any} />);
    expect(
      screen.getByTestId("selective-disclosure-builder"),
    ).toBeInTheDocument();
  });
  ...
  it("renders cancel button on first step", () => {
    ...
    render(
      <VerificationFlow request={mockRequest as any} onCancel={onCancel} />,
    );
  ...
  it("submits proof and shows success result", async () => {
    ...
    render(
      <VerificationFlow request={mockRequest as any} onComplete={onComplete} />,
    );
    ...
    expect(
      screen.getByText(/zero-knowledge proof has been verified/),
    ).toBeInTheDocument();
  ...
  it("shows verification failed result", async () => {
    jest
      .spyOn(require("@/hooks/useVerification"), "useVerification")
      .mockReturnValue({
        submitProof: jest
          .fn()
          .mockResolvedValue({ verified: false, reason: "Invalid proof" }),
        isVerifying: false,
      });
  ...
  it('shows "No attributes directly disclosed" when none are disclosed', () => {
    ...
    fireEvent.click(screen.getByText("Mock Generate Proof"));
    expect(
      screen.getByText("No attributes directly disclosed"),
    ).toBeInTheDocument();
  });
  ...
  it("passes onError to ProofGenerator on generate-proof step (covers line 227)", () => {
    ...
    jest
      .spyOn(require("@/components/zkp/ProofGenerator"), "default")
      .mockImplementation(({ onError }: any) => (
        <div data-testid="proof-generator">
          <button onClick={() => onError("proof failed")}>Mock Error</button>
        </div>
      ));
    ...
    jest
      ...
      .mockImplementation(({ onComplete }: any) => (
        <div data-testid="selective-disclosure-builder">
          <button
            onClick={() =>
              onComplete({ disclosed: [], zkProved: [], hidden: [] })
            }
          >
            Mock Complete Selection
          </button>
