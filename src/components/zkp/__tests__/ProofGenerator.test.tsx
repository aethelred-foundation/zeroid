...
describe('ProofGenerator', () => {
  ...
  it('renders idle state', () => {
    render(<ProofGenerator disclosure={mockDisclosure as any} onProofGenerated={onProofGenerated} onError={onError} />);
    expect(screen.getByText('Ready')).toBeInTheDocument();
    expect(screen.getByText('Click generate to begin ZK proof creation')).toBeInTheDocument();
  });
  ...
  it('renders stage step labels', () => {
    render(<ProofGenerator disclosure={mockDisclosure as any} onProofGenerated={onProofGenerated} onError={onError} />);
    expect(screen.getAllByText('Loading Circuit').length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText('Computing Witness').length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText('Generating Proof').length).toBeGreaterThanOrEqual(1);
  });
  ...
  it('shows generic error message for non-Error throw (line 94)', async () => {
    ...
    await waitFor(() => {
      expect(onError).toHaveBeenCalledWith('Unknown error during proof generation');
    });
    expect(screen.getByText('Unknown error during proof generation')).toBeInTheDocument();
  });
  ...
  it('renders circuit nodes with highlighted fill during generating-proof stage (covers idx >= 4 branch)', async () => {
    ...
    const deferredProof = new Promise((resolve) => { resolveProof = resolve; });
