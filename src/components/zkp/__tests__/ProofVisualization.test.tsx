...
describe('ProofVisualization', () => {
  it('renders proof hash', () => {
    ...
    expect(screen.getByText('0xabcdef1234567890abcdef1234567890')).toBeInTheDocument();
  });
  ...
  it('handles copy hash action', async () => {
    ...
    expect(writeTextMock).toHaveBeenCalledWith('0xabcdef1234567890abcdef1234567890');
  });
  ...
  it('runs verifying interval and cleans up (covers useEffect interval)', () => {
    ...
    rerender(<ProofVisualization proof={mockProof as any} isVerifying={false} isVerified={true} />);
    expect(screen.getByText('Proof Verified')).toBeInTheDocument();
    jest.useRealTimers();
  });
  ...
  it('sets ring progress to 100 when isVerified changes to true', () => {
    const { rerender } = render(
      <ProofVisualization proof={mockProof as any} isVerifying={false} isVerified={false} />
    );
    rerender(<ProofVisualization proof={mockProof as any} isVerifying={false} isVerified={true} />);
    expect(screen.getByText('Proof Verified')).toBeInTheDocument();
  });
