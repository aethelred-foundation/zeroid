...
const voteConfig: Record<
  VoteChoice,
  { label: string; icon: typeof ThumbsUp; color: string; bgColor: string; hoverBg: string }
> = {
  for: {
    label: 'Vote For',
    icon: ThumbsUp,
...
export default function VotingPanel({ proposal, onVoteSubmitted }: VotingPanelProps) {
  ...
  const { vote, delegate, votingPower, delegatedTo, isLoading: governanceLoading } = useGovernance();
      ...
      <div className="p-5 border-b border-[var(--border-primary)]">
        ...
        {delegatedTo && (
          <div className="mt-2 flex items-center gap-1 text-xs text-[var(--text-tertiary)]">
            ...
            Delegated to: <span className="font-mono">{delegatedTo.slice(0, 6)}...{delegatedTo.slice(-4)}</span>
          </div>
        )}
              ...
              <div className="p-3 rounded-xl bg-[var(--surface-secondary)] border border-[var(--border-primary)]">
                <div className="flex items-start gap-2 mb-3">
                  ...
                  <p className="text-xs text-[var(--text-secondary)]">
                    Delegate your voting power to another address. You can reclaim it at any time.
                  </p>
                </div>
                <input
                  type="text"
                ...
                />
                <button onClick={handleDelegate} className="btn-primary btn-sm w-full">
      ...
      <div className="p-5">
        ...
        {hasVoted ? (
          ...
          >
            ...
            <p className="text-sm font-semibold text-[var(--text-primary)]">Vote Submitted</p>
            <p className="text-xs text-[var(--text-secondary)] mt-1">
              You voted <strong className="capitalize">{selectedVote}</strong> on this proposal.
            </p>
          </motion.div>
        ) : !isActive ? (
          ...
          <div className="space-y-3">
            {(Object.entries(voteConfig) as [VoteChoice, typeof voteConfig.for][]).map(
              ([choice, config]) => {
                  ...
                  >
                    <div
                      className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                        isSelected ? config.bgColor : 'bg-[var(--surface-secondary)]'
                      }`}
                    >
