...
export default function ProposalCard({ proposal, onVote, onViewDetails }: ProposalCardProps) {
  const [hasVoted, setHasVoted] = useState<'for' | 'against' | 'abstain' | null>(null);
  ...
  const quorumPercentage = proposal.quorum > 0
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className={`badge ${status.bgColor} ${status.color} border-0`}>
        ...
        {hasVoted && (
          <div className="flex items-center gap-2 p-2 rounded-lg bg-brand-500/5 text-sm">
            ...
            <span className="text-[var(--text-secondary)]">
              You voted <strong className="text-[var(--text-primary)] capitalize">{hasVoted}</strong>
            </span>
          </div>
        )}
