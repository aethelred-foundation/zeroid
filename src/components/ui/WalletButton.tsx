...
function VerificationBadge({ level }: { level: VerificationLevel }) {
  const config: Record<VerificationLevel, { icon: React.ReactNode; className: string; label: string }> = {
    verified: {
  ...
  return (
    <span className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-[9px] font-medium border ${className}`} title={label}>
      {icon}
    </span>
  );
}
  ...
  return (
    <ConnectButton.Custom>
      {({ account, chain, openAccountModal, openChainModal, openConnectModal, authenticationStatus, mounted }) => {
        ...
        if (!ready) {
          return <div className={`h-9 w-[130px] rounded-xl animate-pulse ${className}`} style={{ background: 'rgba(255,255,255,0.04)' }} />;
        }
          ...
          return (
            <button
              ...
              style={{
                ...
                color: '#0a0b0d',
                boxShadow: '0 1px 2px rgba(0,0,0,0.3), 0 0 0 1px rgba(255,255,255,0.1) inset, 0 1px 0 rgba(255,255,255,0.15) inset',
              }}
            >
        ...
        const verificationLevel: VerificationLevel = chain.unsupported ? 'unverified' : 'verified';
        ...
        return (
          <div className={`flex items-center gap-1.5 ${className}`}>
            {chain.unsupported ? (
              ...
              >
                {chain.hasIcon && chain.iconUrl && (
                  <img src={chain.iconUrl} alt={chain.name ?? 'Chain'} className="w-3.5 h-3.5 rounded-full" />
                )}
                {chain.name}
              </button>
            )}
            ...
            >
              <span className="text-zero-200 font-medium font-mono text-[11px]">{account.displayName}</span>
              <VerificationBadge level={verificationLevel} />
              {account.displayBalance && (
                <span className="hidden md:inline text-[11px] text-zero-500 font-mono">{account.displayBalance}</span>
              )}
