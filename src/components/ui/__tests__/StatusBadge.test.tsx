...
jest.mock('lucide-react', () => ({
  ...
  ShieldOff: (props: Record<string, unknown>) => <span data-testid="icon-shield-off" {...props} />,
...
describe('StatusBadge', () => {
  ...
  it('shows dot indicator when showDot is true', () => {
    ...
    const dotSpans = container.querySelectorAll('.relative.flex.h-1\\.5.w-1\\.5');
  ...
  it('applies custom className', () => {
    const { container } = render(<StatusBadge status="verified" className="custom-class" />);
    const badge = container.firstChild as HTMLElement;
  ...
  it('renders correct icon for each status', () => {
    ...
    for (const [status, testId] of Object.entries(iconMap)) {
      const { unmount } = render(<StatusBadge status={status as CredentialStatus} />);
...
describe('VerificationBadge', () => {
  ...
  it('applies custom className when verified', () => {
    const { container } = render(<VerificationBadge verified={true} className="test-class" />);
    const badge = container.firstChild as HTMLElement;
    expect(badge.className).toContain('test-class');
  });

  it('applies custom className when unverified', () => {
    const { container } = render(<VerificationBadge verified={false} className="test-class" />);
    const badge = container.firstChild as HTMLElement;
    expect(badge.className).toContain('test-class');
  });
