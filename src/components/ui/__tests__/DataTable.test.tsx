...
describe('DataTable', () => {
  ...
  it('shows empty state when data is empty', () => {
    render(
      <DataTable columns={columns} data={[]} keyExtractor={(_, i) => String(i)} />
    );
  ...
  it('shows loading skeleton when loading is true', () => {
    const { container } = render(
      <DataTable columns={columns} data={data} keyExtractor={(row) => row.id} loading />
    );
  ...
  it('calls onRowClick when a row is clicked', () => {
    ...
    render(
      <DataTable columns={columns} data={data} keyExtractor={(row) => row.id} onRowClick={onRowClick} />
    );
  ...
  describe('sorting', () => {
    ...
    it('does not sort when sortable prop is false', () => {
      const { container } = render(
        <DataTable columns={columns} data={data} keyExtractor={(row) => row.id} sortable={false} />
      );
      fireEvent.click(screen.getByText('Name'));
      const cells = container.querySelectorAll('tbody td:first-child');
      const names = Array.from(cells).map((c) => c.textContent);
    ...
    it('does not sort non-sortable columns', () => {
      ...
      const { container } = render(
        <DataTable columns={colsWithNonSortable} data={data} keyExtractor={(row) => row.id} />
      );
      fireEvent.click(screen.getByText('Name'));
      const cells = container.querySelectorAll('tbody td:first-child');
      const names = Array.from(cells).map((c) => c.textContent);
    ...
    it('sorts using custom accessor', () => {
      const colsWithAccessor: Column<TestRow>[] = [
        { key: 'name', header: 'Name', sortable: true, accessor: (row) => row.name.toLowerCase() },
        { key: 'age', header: 'Age', sortable: true },
      ];
      const { container } = render(
        <DataTable columns={colsWithAccessor} data={data} keyExtractor={(row) => row.id} />
      );
      fireEvent.click(screen.getByText('Name'));
      const cells = container.querySelectorAll('tbody td:first-child');
      const names = Array.from(cells).map((c) => c.textContent);
    ...
    it('handles null values in sort (pushes nulls to end)', () => {
      interface NullRow { id: string; val: string | null }
      ...
      const { container } = render(
        <DataTable columns={nullCols} data={nullData} keyExtractor={(row) => row.id} />
      );
    ...
    it('handles mixed type comparison (falls back to string)', () => {
      interface MixedRow { id: string; val: unknown }
      ...
      const { container } = render(
        <DataTable columns={mixedColsWithAccessor} data={mixedData} keyExtractor={(row) => row.id} />
      );
      fireEvent.click(screen.getByText('Val'));
      const cells = container.querySelectorAll('tbody td');
      const vals = Array.from(cells).map((c) => c.textContent);
    ...
    it('resets page to 1 when sorting', () => {
      ...
      render(
        <DataTable columns={columns} data={bigData} keyExtractor={(row) => row.id} pageSize={5} />
      );
      // Go to page 2
  ...
  describe('SortIcon', () => {
    it('shows unsorted icon by default', () => {
      ...
      expect(screen.getAllByTestId('icon-chevronsupdown').length).toBeGreaterThan(0);
    });
  ...
  describe('pagination', () => {
    ...
    it('does not show pagination when data fits in one page', () => {
      render(
        <DataTable columns={columns} data={data} keyExtractor={(row) => row.id} pageSize={10} />
      );
    ...
    it('shows pagination when data exceeds one page', () => {
      render(
        <DataTable columns={columns} data={bigData} keyExtractor={(row) => row.id} pageSize={10} />
      );
    ...
    it('shows ellipsis for many pages', () => {
      ...
      render(
        <DataTable columns={columns} data={manyData} keyExtractor={(row) => row.id} pageSize={10} />
      );
    ...
    it('shows no ellipsis when totalPages <= 7', () => {
      ...
      render(
        <DataTable columns={columns} data={smallData} keyExtractor={(row) => row.id} pageSize={5} />
      );
      // 7 pages exactly
  ...
  describe('column alignment', () => {
    it('applies text-center for center alignment', () => {
      ...
      const { container } = render(
        <DataTable columns={centeredCols} data={data} keyExtractor={(row) => row.id} />
      );
    ...
    it('applies text-right for right alignment', () => {
      ...
      const { container } = render(
        <DataTable columns={rightCols} data={data} keyExtractor={(row) => row.id} />
      );
      const th = container.querySelector('th');
  ...
  describe('custom render and accessor', () => {
    it('uses custom render function for cell display', () => {
      ...
      render(
        <DataTable columns={customCols} data={data} keyExtractor={(row) => row.id} />
      );
    ...
    it('passes correct arguments to render function', () => {
      const renderFn = jest.fn((value, row, index) => <span>{String(value)}</span>);
      const customCols: Column<TestRow>[] = [
  ...
  describe('column width', () => {
    it('applies width style to header and body cells', () => {
      ...
      const { container } = render(
        <DataTable columns={widthCols} data={data} keyExtractor={(row) => row.id} />
      );
      const th = container.querySelector('th');
  ...
  describe('stickyHeader', () => {
    it('adds sticky class when stickyHeader is true', () => {
      const { container } = render(
        <DataTable columns={columns} data={data} keyExtractor={(row) => row.id} stickyHeader />
      );
  ...
  describe('className', () => {
    it('applies custom className to the wrapper', () => {
      const { container } = render(
        <DataTable columns={columns} data={data} keyExtractor={(row) => row.id} className="my-custom-class" />
      );
    ...
    it('applies custom className in loading state', () => {
      const { container } = render(
        <DataTable columns={columns} data={data} keyExtractor={(row) => row.id} loading className="loading-class" />
      );
    ...
    it('applies custom className in empty state', () => {
      const { container } = render(
        <DataTable columns={columns} data={[]} keyExtractor={(_, i) => String(i)} className="empty-class" />
      );
  ...
  describe('row click styling', () => {
    it('applies cursor-pointer class when onRowClick is provided', () => {
      const { container } = render(
        <DataTable columns={columns} data={data} keyExtractor={(row) => row.id} onRowClick={() => {}} />
      );
  ...
  describe('keyExtractor', () => {
    it('uses index-based keyExtractor', () => {
      render(
        <DataTable columns={columns} data={data} keyExtractor={(_, i) => String(i)} />
      );
      expect(screen.getByText('Alice')).toBeInTheDocument();
    });
  });
  ...
  describe('sort icon visibility', () => {
    it('does not show sort icon on non-sortable column', () => {
      ...
      render(
        <DataTable columns={mixedCols} data={data} keyExtractor={(row) => row.id} />
      );
  ...
  describe('accessor in body cells', () => {
    it('uses accessor to get cell value (not key lookup)', () => {
      ...
      render(
        <DataTable columns={accessorCols} data={data} keyExtractor={(row) => row.id} />
      );
      expect(screen.getByText('Alice (30)')).toBeInTheDocument();
  ...
  describe('handleSort edge cases', () => {
    it('does not sort when column.sortable is false even if table sortable is true (line 199)', () => {
      ...
      const { container } = render(
        <DataTable columns={colsExplicit} data={data} keyExtractor={(row) => row.id} sortable={true} />
      );
    ...
    it('handles sort on column with no matching column in columns array (line 218)', () => {
      ...
      rerender(
        <DataTable columns={newCols} data={data} keyExtractor={(row) => row.id} />
      );
      const cells = container.querySelectorAll('tbody td');
    ...
    it('covers the !sortable early return in handleSort (line 199)', () => {
      ...
      const { rerender, container } = render(
        <DataTable columns={columns} data={data} keyExtractor={(row) => row.id} sortable={true} />
      );
  ...
  describe('loading skeleton', () => {
    it('renders 5 skeleton rows', () => {
      const { container } = render(
        <DataTable columns={columns} data={[]} keyExtractor={(_, i) => String(i)} loading />
      );
