  ...
  if (loading) {
    return (
      <div className={`bento p-6 ${className}`} aria-hidden="true">
        <div className="w-10 h-10 rounded-xl animate-pulse mb-5" style={{ background: 'rgba(255,255,255,0.04)' }} />
        <div className="w-20 h-3 rounded-lg animate-pulse mb-3" style={{ background: 'rgba(255,255,255,0.04)' }} />
        <div className="w-16 h-8 rounded-lg animate-pulse" style={{ background: 'rgba(255,255,255,0.04)' }} />
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Label */}
        <p className="text-label-sm text-zero-500 mb-2 uppercase font-body">{label}</p>

        {/* Value — Big and bold */}
        <div className="flex items-end justify-between">
          <p className="text-[36px] font-bold text-white tracking-tight font-display leading-none">{value}</p>

          {trendConfig && trend && (
            <div className={`flex items-center gap-1 text-[11px] font-medium ${trendConfig.color} pb-1`}>
              {trendConfig.icon}
        ...
        {(subtitle || trend?.label) && (
          <p className="text-[11px] text-zero-500 mt-2.5 font-body">{subtitle || trend?.label}</p>
        )}
      </Component>
    </motion.div>
  );
...
export function MetricCardGrid({ children, columns = 4, className = '' }: MetricCardGridProps) {
  return <div className={`grid ${GRID_COLS[columns]} gap-4 ${className}`}>{children}</div>;
}
