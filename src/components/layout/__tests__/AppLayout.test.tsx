...
jest.mock("../Sidebar", () => ({
  Sidebar: ({ className, mobile, onToggle }: any) => (
    ...
    >
      Sidebar
      <button
        data-testid={mobile ? "mobile-sidebar-close" : "desktop-sidebar-toggle"}
        onClick={onToggle}
      >
        {mobile ? "Close" : "Toggle"}
      </button>
...
describe("NAV_SECTIONS", () => {
  ...
  it("some items have badges", () => {
    ...
    expect(badgedItems.find((i) => i.label === "AI Compliance")?.badge).toBe(
      "AI",
    );
    expect(badgedItems.find((i) => i.label === "Agent Identity")?.badge).toBe(
      "New",
    );
  });
});
