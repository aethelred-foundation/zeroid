...
jest.mock('@rainbow-me/rainbowkit', () => ({
  ConnectButton: {
    Custom: ({ children }: { children: (props: any) => React.ReactNode }) => {
      return <>{children({ account: undefined, chain: undefined, openAccountModal: jest.fn(), openChainModal: jest.fn(), openConnectModal: jest.fn(), mounted: true })}</>;
    },
  },
}));
...
jest.mock('lucide-react', () => {
  const createIcon = (name: string) => (props: Record<string, unknown>) => <span data-testid={`icon-${name}`} {...props} />;
  return {
...
describe('Sidebar', () => {
  ...
  describe('Mobile mode', () => {
    ...
    it('applies custom className', () => {
      const { container } = render(<Sidebar {...defaultProps} mobile className="custom-sidebar" />);
  ...
  describe('Desktop mode (dock)', () => {
    ...
    it('applies custom className to dock', () => {
      const { container } = render(<Sidebar {...defaultProps} className="dock-custom" />);
      const aside = container.querySelector('aside');
