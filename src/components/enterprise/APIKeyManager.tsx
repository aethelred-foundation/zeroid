...
> = {
  read: {
    ...
    description: "Read identity and credential data",
  },
  write: {
    label: "Write",
  ...
  verify: {
    ...
    description: "Verify proofs and credentials",
  },
  issue: {
    label: "Issue",
...
const DEFAULT_KEYS: APIKey[] = [
  ...
  },
  {
    ...
    keyPrefix: "zid_test_b2c1...",
    scopes: ["read", "write", "verify", "issue"],
...
function CreateKeyModal({
  ...
  const [selectedScopes, setSelectedScopes] = useState<Set<KeyScope>>(
    new Set(["read"]),
  );
      ...
      >
        ...
        <div className="flex items-center gap-2 p-5 border-t border-[var(--border-primary)]">
          ...
          <button
            onClick={() => {
              if (name.trim())
                onSubmit(name.trim(), Array.from(selectedScopes));
            }}
            disabled={!name.trim()}
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <button
                    onClick={() =>
                      handleCopy(key.fullKey ?? key.keyPrefix, key.id)
                    }
                    className="p-1 rounded hover:bg-[var(--surface-secondary)] transition-colors"
                    aria-label="Copy key"
                  >
                ...
                <div className="mt-2">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-[10px] text-[var(--text-tertiary)]">
                      {key.rateLimitUsed.toLocaleString()} /{" "}
                  ...
                  <div className="w-full h-1.5 rounded-full bg-[var(--surface-tertiary)]">
                    <div
                      className={`h-full rounded-full transition-all ${
                        key.rateLimitUsed / key.rateLimit > 0.8
                          ? "bg-red-500"
                          : key.rateLimitUsed / key.rateLimit > 0.5
                            ? "bg-amber-500"
                            : "bg-emerald-500"
                      }`}
                      style={{
                        width: `${Math.min((key.rateLimitUsed / key.rateLimit) * 100, 100)}%`,
                      }}
                    />
                  </div>
                </div>
