...
describe('APIKeyManager', () => {
  ...
  it('calls onCreateKey when creating a key via modal', async () => {
    ...
    fireEvent.click(screen.getByText('Create Key'));
    // Fill in name
    fireEvent.change(screen.getByPlaceholderText('e.g., Production API'), { target: { value: 'Test Key' } });
  ...
  it('toggles scope selection in create modal', () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('e.g., Production API'), { target: { value: 'Scoped Key' } });
    const createButtons = screen.getAllByText('Create Key');
    // Should not throw
    fireEvent.click(createButtons[createButtons.length - 1]);
  });
  ...
  it('closes create modal by clicking overlay backdrop', () => {
    ...
    const backdrop = screen.getByText('Create API Key').closest('[class*="fixed"]')!;
  ...
  it('closes confirm modal by clicking overlay backdrop', () => {
    ...
    expect(screen.getByText('Delete API Key')).toBeInTheDocument();
    // Click backdrop
    const backdrop = screen.getByText('Delete API Key').closest('[class*="fixed"]')!;
    fireEvent.click(backdrop);
  ...
  it('handles handleCreate without onCreateKey callback', async () => {
    render(<APIKeyManager keys={mockKeys} />);
    fireEvent.click(screen.getByText('Create Key'));
    fireEvent.change(screen.getByPlaceholderText('e.g., Production API'), { target: { value: 'New Key' } });
    const createButtons = screen.getAllByText('Create Key');
    fireEvent.click(createButtons[createButtons.length - 1]);
  ...
  it('shows correct rate limit bar colors', () => {
    const keysWithDifferentRates = [
      ...
      { ...mockKeys[1], id: 'k3', name: 'Mid Key', rateLimitUsed: 600, rateLimit: 1000, active: true }, // > 50%, amber
    ];
  ...
  it('applies className to loading state', () => {
    const { container } = render(<APIKeyManager loading={true} className="load-class" />);
    expect(container.firstChild).toHaveClass('load-class');
  });

  it('applies className to error state', () => {
    const { container } = render(<APIKeyManager error="err" className="err-class" />);
  ...
  it('prevents deselecting last scope in create modal', () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('e.g., Production API'), { target: { value: 'Test' } });
    const createButtons = screen.getAllByText('Create Key');
    fireEvent.click(createButtons[createButtons.length - 1]);
  ...
  it('handleConfirmAction returns early when confirmAction is null', async () => {
    ...
    const spy = jest.spyOn(React, 'useCallback').mockImplementation((fn: any, deps: any) => {
  ...
  it('shows key prefix when not visible even if fullKey exists', () => {
    ...
    expect(screen.getByText('zid_live_a8f3...')).toBeInTheDocument();
    expect(screen.queryByText('zid_live_a8f3_full_key_here')).not.toBeInTheDocument();
  });
