      ...
      >
        ...
        <div className="flex items-center gap-2 p-5 border-t border-[var(--border-primary)]">
          ...
          <button
            onClick={() => {
              if (isValid)
                onSubmit(url.trim(), Array.from(selectedEvents), secret);
            }}
            disabled={!isValid}
    ...
    >
      ...
      >
        <div className="flex items-start gap-3">
          ...
          >
            <Globe
              className={`w-4 h-4 ${webhook.active ? "text-brand-500" : "text-[var(--text-tertiary)]"}`}
            />
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span
                className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium ${statusConfig.color}`}
              >
                <span
              ...
              <div className="space-y-1.5">
                {webhook.deliveryLogs.map((log) => {
                  ...
                  return (
                    ...
                    >
                      <StatusIcon
                        className={`w-4 h-4 ${statusCfg.color} flex-shrink-0`}
                      />
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {log.statusCode && (
                            <span
                              className={`text-[10px] font-mono ${log.statusCode < 400 ? "text-emerald-400" : "text-red-400"}`}
                            >
                              {log.statusCode}
                            </span>
                          )}
