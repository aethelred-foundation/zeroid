    ...
    get: (_target: unknown, prop: string) => {
      if (typeof prop === 'string') {
        const Component = React.forwardRef((props: any, ref: any) => {
          const { initial, animate, exit, transition, whileHover, whileTap, variants, layout, onHoverStart, onHoverEnd, ...domProps } = props;
...
describe('IdentityCard', () => {
  ...
  it('shows loading skeleton when hook isLoading', () => {
    mockUseIdentity.mockReturnValue({ identity: null, isLoading: true, error: null });
    const { container } = render(<IdentityCard />);
    expect(container.querySelector('.animate-pulse')).toBeInTheDocument();
  });

  it('shows error message when hook returns error', () => {
    mockUseIdentity.mockReturnValue({ identity: null, isLoading: false, error: new Error('Network error') });
    render(<IdentityCard />);
  ...
  it('uses context identity when no prop is passed', () => {
    mockUseIdentity.mockReturnValue({ identity: mockIdentity, isLoading: false, error: null });
    render(<IdentityCard />);
    expect(screen.getByText('ZeroID')).toBeInTheDocument();
    expect(screen.getByText('5')).toBeInTheDocument();
  });
  ...
  it('prefers prop identity over context identity', () => {
    ...
    mockUseIdentity.mockReturnValue({ identity: contextIdentity, isLoading: false, error: null });
    render(<IdentityCard identity={mockIdentity as any} />);
    expect(screen.getByText('5')).toBeInTheDocument();
  ...
  it('displays error message text from error object', () => {
    mockUseIdentity.mockReturnValue({ identity: null, isLoading: false, error: new Error('Timeout') });
    render(<IdentityCard />);
    expect(screen.getByText(/Timeout/)).toBeInTheDocument();
  });

  it('renders pending verification status', () => {
    const pendingIdentity = { ...mockIdentity, verificationStatus: 'pending' as const };
    render(<IdentityCard identity={pendingIdentity as any} />);
    expect(screen.getByText('Pending')).toBeInTheDocument();
  });

  it('renders revoked verification status', () => {
    const revokedIdentity = { ...mockIdentity, verificationStatus: 'revoked' as const };
    render(<IdentityCard identity={revokedIdentity as any} />);
    expect(screen.getByText('Revoked')).toBeInTheDocument();
  });

  it('renders expired verification status', () => {
    const expiredIdentity = { ...mockIdentity, verificationStatus: 'expired' as const };
    render(<IdentityCard identity={expiredIdentity as any} />);
    expect(screen.getByText('Expired')).toBeInTheDocument();
  });

  it('renders unverified verification status', () => {
    const unverifiedIdentity = { ...mockIdentity, verificationStatus: 'unverified' as const };
  ...
  it('renders onViewDetails button in full view and calls it', () => {
    ...
    render(<IdentityCard identity={mockIdentity as any} onViewDetails={onViewDetails} />);
  ...
  it('calls onViewDetails in compact mode when card is clicked', () => {
    const onViewDetails = jest.fn();
    render(<IdentityCard identity={mockIdentity as any} compact onViewDetails={onViewDetails} />);
    // In compact mode, the whole card is clickable
    const card = screen.getByText('Verified').closest('div[class*="card-interactive"]');
    if (card) {
  ...
  it('falls back to unverified status for unknown verificationStatus', () => {
    const unknownStatusIdentity = { ...mockIdentity, verificationStatus: 'unknown_status' as any };
