...
describe('IdentityCreation', () => {
  ...
  it('renders the first step (Connect Wallet)', () => {
    ...
    expect(screen.getByText('Link your Web3 wallet to anchor your identity')).toBeInTheDocument();
  });
  ...
  it('shows verified state for UAE Pass', () => {
    ...
    expect(screen.queryByText('Start UAE Pass Verification')).not.toBeInTheDocument();
  });
  ...
  it('handles UAE Pass error with Error instance', async () => {
    mockInitiateVerification.mockRejectedValueOnce(new Error('UAE Pass timeout'));
    render(<IdentityCreation />);
    navigateToStep(1);
    await act(async () => {
      fireEvent.click(screen.getByText('Start UAE Pass Verification'));
  ...
  it('handles UAE Pass error with non-Error value', async () => {
    ...
    expect(screen.getByText('UAE Pass verification failed')).toBeInTheDocument();
  });
  ...
  it('shows Generate DID step content', () => {
    render(<IdentityCreation />);
    navigateToStep(3);
    expect(screen.getByText('Generate Decentralized Identifier')).toBeInTheDocument();
    expect(screen.getAllByText('Generate DID').length).toBeGreaterThanOrEqual(1);
  });
  ...
  it('calls createIdentity when Generate DID button is clicked', async () => {
    ...
    const actionButton = genButtons.find((el) => el.closest('button')?.className.includes('btn-primary'));
    await act(async () => {
  ...
  it('handles DID generation error with Error instance', async () => {
    mockCreateIdentity.mockRejectedValueOnce(new Error('Key derivation failed'));
    render(<IdentityCreation />);
    navigateToStep(3);
    const genButtons = screen.getAllByText('Generate DID');
    const actionButton = genButtons.find((el) => el.closest('button')?.className.includes('btn-primary'));
  ...
  it('shows On-Chain Registration step content', () => {
    render(<IdentityCreation />);
    navigateToStep(4);
    expect(screen.getAllByText('On-Chain Registration').length).toBeGreaterThanOrEqual(1);
  ...
  it('handles on-chain registration error with Error instance', async () => {
    mockRegisterOnChain.mockRejectedValueOnce(new Error('Transaction reverted'));
    render(<IdentityCreation />);
    navigateToStep(4);
    await act(async () => {
      fireEvent.click(screen.getByText('Register On-Chain'));
  ...
  it('handles on-chain registration error with non-Error value', async () => {
    ...
    expect(screen.getByText('On-chain registration failed')).toBeInTheDocument();
  });
  ...
  it('shows Generating... text while DID is being generated', async () => {
    ...
    mockCreateIdentity.mockImplementation(() => new Promise<void>((resolve) => { resolveCreate = resolve; }));
  ...
  it('shows Registering... text while on-chain registration is in progress', async () => {
    ...
    mockRegisterOnChain.mockImplementation(() => new Promise<void>((resolve) => { resolveRegister = resolve; }));
  ...
  it('shows loader icon while UAE Pass verification is processing', async () => {
    let resolveUAE: () => void;
    mockInitiateVerification.mockImplementation(() => new Promise<void>((resolve) => { resolveUAE = resolve; }));
    ...
    const button = screen.getByText('Start UAE Pass Verification').closest('button');
  ...
  it('renderStepContent returns null for out-of-range step (default case)', () => {
    ...
    const fiberKey = Object.keys(rootDiv).find((key) => key.startsWith('__reactFiber$'));
    if (fiberKey) {
