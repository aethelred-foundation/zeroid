...
describe('PrivacyScoreBreakdown', () => {
  it('renders loading state', () => {
    ...
    expect(screen.getByText('Calculating privacy score...')).toBeInTheDocument();
  });
  ...
  it('renders category breakdown', () => {
    ...
    expect(screen.getAllByText('Data Disclosure').length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText('ZK Usage').length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText('Verifier Diversity').length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText('Credential Freshness').length).toBeGreaterThanOrEqual(1);
  });
  ...
  it('renders recommendations (initially shows 2)', () => {
    ...
    expect(screen.getByText('Enable ZK proofs for age verification')).toBeInTheDocument();
    expect(screen.getByText('Diversify verifier interactions')).toBeInTheDocument();
  });
  ...
  it('shows all recommendations when "Show more" is clicked', () => {
    ...
    expect(screen.getByText('Use selective disclosure for employment')).toBeInTheDocument();
  });
  ...
  it('applies custom className', () => {
    const { container } = render(<PrivacyScoreBreakdown className="test-class" />);
  ...
  it('renders CategoryBar with percentage < 60 showing red bar', () => {
    const lowCategories = [
      { id: 'low', name: 'Low Score Cat', score: 30, maxScore: 100, description: 'Very low', icon: 'disclosure' as const },
    ];
  ...
  it('renders CategoryBar with percentage between 60-80 showing amber bar', () => {
    const midCategories = [
      { id: 'mid', name: 'Mid Score Cat', score: 65, maxScore: 100, description: 'Medium', icon: 'zk' as const },
    ];
  ...
  it('renders CategoryBar with unknown icon falling back to Shield', () => {
    const unknownIconCategories = [
      { id: 'unk', name: 'Unknown Icon', score: 90, maxScore: 100, description: 'Test', icon: 'nonexistent' as any },
    ];
  ...
  it('does not show "Show more" when recommendations <= 2', () => {
    const fewRecs = [
      { id: 'r1', title: 'Rec 1', description: 'Desc 1', impact: 'high' as const, category: 'Cat' },
    ];
  ...
  it('renders loading state with custom className', () => {
    const { container } = render(<PrivacyScoreBreakdown loading={true} className="loading-class" />);
    expect(container.firstChild).toHaveClass('loading-class');
  });

  it('renders error state with custom className', () => {
    const { container } = render(<PrivacyScoreBreakdown error="err" className="error-class" />);
