...
const mockCredentials = [
  { id: 'c1', name: 'KYC Level 2', issuer: 'Aethelred', schema: 'kyc-v2', expiresAt: '2027-01-15' },
...
describe('BridgeInterface', () => {
  ...
  it('renders the bridge header', () => {
    ...
    expect(screen.getByText('Cross-Chain Credential Bridge')).toBeInTheDocument();
  ...
  it('opens chain selector dropdown and selects a chain', () => {
    ...
    const sourceChainButton = screen.getByText('Source Chain')
      .closest('div')!
  ...
  it('starts bridging flow when bridge button is clicked', async () => {
    ...
    render(<BridgeInterface credentials={mockCredentials} onBridge={onBridge} />);
  ...
  it('opens destination chain selector dropdown', () => {
    render(<BridgeInterface />);
    const destChainButton = screen.getByText('Destination Chain')
      .closest('div')!
      .querySelector('button')!;
  ...
  it('applies className to loading state', () => {
    const { container } = render(<BridgeInterface loading={true} className="custom-loading" />);
    expect(container.firstChild).toHaveClass('custom-loading');
  });

  it('applies className to error state', () => {
    const { container } = render(<BridgeInterface error="fail" className="custom-error" />);
  ...
  it('renders step descriptions in BridgeStepper', async () => {
    ...
    expect(screen.getByText('Relayer transmitting credential proof')).toBeInTheDocument();
    expect(screen.getByText('Credential available on destination')).toBeInTheDocument();
  ...
  it('handleBridge returns early when no credentials are selected', async () => {
    ...
    const { container } = render(<BridgeInterface credentials={mockCredentials} onBridge={onBridge} />);

    // Find the bridge button and get its React props via internal fiber
    const bridgeButton = screen.getByText(/Bridge 0 Credential/).closest('button')!;
    // Get the React fiber node
    const fiberKey = Object.keys(bridgeButton).find(key => key.startsWith('__reactFiber$') || key.startsWith('__reactInternalInstance$'));
    if (fiberKey) {
