  ...
  [
    {
      key: "initiate",
      label: "Initiate",
      description: "Submit bridge request",
    },
    {
      key: "confirm_source",
      label: "Source Confirm",
      description: "Transaction confirmed on source chain",
    },
    {
      key: "relay",
      label: "Relay",
      description: "Relayer transmitting credential proof",
    },
    {
      key: "confirm_dest",
      label: "Dest Confirm",
      description: "Transaction confirmed on destination",
    },
    {
      key: "complete",
      label: "Complete",
      description: "Credential available on destination",
    },
  ];
...
const DEFAULT_CREDENTIALS: BridgeableCredential[] = [
  {
    id: "c1",
    name: "KYC Level 2 Verification",
    issuer: "Aethelred Identity",
    schema: "kyc-v2",
  ...
  },
  {
    ...
    issuer: "SEC Registry",
    schema: "accredited-investor-v1",
    expiresAt: "2026-12-01",
  },
  {
    id: "c3",
    name: "AML Compliance Certificate",
    issuer: "ComplianceOracle",
    schema: "aml-cert-v1",
  },
  {
    id: "c4",
        ...
        {open && (
          ...
          >
            {CHAINS.filter((c) => c.id !== excludeChain).map((c) => (
              ...
              >
                <div
                  className={`w-6 h-6 rounded-md bg-gradient-to-br ${c.color} flex items-center justify-center text-white text-[10px] font-bold`}
                >
                  {c.icon}
                </div>
                ...
                {c.id === selected && (
                  <CheckCircle2 className="w-3.5 h-3.5 text-brand-500 ml-auto" />
                )}
              </button>
            ))}
          </motion.div>
        ...
        return (
          ...
          >
            ...
            <div className="flex-1">
              <p
                className={`text-xs font-medium ${isCurrent ? "text-brand-500" : isComplete ? "text-emerald-400" : "text-[var(--text-tertiary)]"}`}
              >
                {step.label}
              </p>
...
export default function BridgeInterface({
  ...
  const fees = useMemo(
    () => getMockFees(sourceChain, destChain),
        ...
        {!bridging ? (
          <>
              ...
              >
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <ChevronDown
                      className={`w-3.5 h-3.5 text-[var(--text-tertiary)] transition-transform ${showFees ? "rotate-180" : ""}`}
                    />
                  </div>
                </button>
                <AnimatePresence>
            ...
            <div className="flex items-start gap-2 p-3 rounded-xl bg-amber-500/5 border border-amber-500/20">
              ...
              <div>
                ...
                <p className="text-[10px] text-[var(--text-secondary)] mt-0.5">
                  Bridged credentials use ZK relay proofs verified by TEE nodes
                  on both chains. The original credential remains valid on the
                  source chain.
                </p>
              </div>
            </div>
            ...
            >
              ...
              Bridge {selectedCredentials.size} Credential
              {selectedCredentials.size !== 1 ? "s" : ""}
            </button>
          </>
        ) : (
          /* Bridge progress */
          <div className="space-y-4">
            <div className="text-center mb-4">
              ...
              <p className="text-xs text-[var(--text-secondary)] mt-1">
                {CHAINS.find((c) => c.id === sourceChain)?.name} to{" "}
