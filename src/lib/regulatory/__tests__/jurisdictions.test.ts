...
describe('JURISDICTIONS', () => {
  const ALL_IDS: JurisdictionId[] = ['uae', 'eu', 'us', 'sg', 'uk', 'bh', 'sa', 'hk', 'jp'];
  ...
  it.each(ALL_IDS)('jurisdiction "%s" credential requirements have valid structure', (id) => {
...
describe('getCrossBorderCompatibility', () => {
  ...
  it('returns null for non-existing pair', () => {
    ...
    const compat = getCrossBorderCompatibility('invalid' as JurisdictionId, 'uae');
    expect(compat).toBeNull();
  });
  ...
  it('returns additional requirements when present', () => {
    ...
    expect(compat).not.toBeNull();
    expect(compat!.additionalRequirements).toContain('Standard Contractual Clauses');
  });
});
...
describe('getJurisdictionsRequiringSchema', () => {
  ...
  it('returns only jurisdictions where schema is mandatory when mandatoryOnly=true', () => {
    ...
    mandatory.forEach((j) => {
      const cred = j.requiredCredentials.find((r) => r.schemaId === 'residency');
...
describe('getStrictestRetentionDays', () => {
  ...
  it('picks correct max when mixing known and unknown', () => {
    const result = getStrictestRetentionDays(['unknown' as JurisdictionId, 'uae']);
