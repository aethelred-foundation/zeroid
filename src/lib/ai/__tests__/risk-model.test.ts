...
describe('extractFeatures', () => {
  ...
  it('sets biometric feature to 1 when enrolled, 0 otherwise', () => {
    const enrolled = extractFeatures(makeIdentityData({ biometricEnrolled: true }));
    expect(enrolled[8]).toBe(1);

    const notEnrolled = extractFeatures(makeIdentityData({ biometricEnrolled: false }));
...
describe('decomposeRiskFactors', () => {
  ...
  it('sets direction to increases_risk for risk-increasing features with value > 0.5', () => {
    ...
    const revokedFactor = factors.find((f) => f.name === 'Revoked Credential Ratio');
...
describe('makeDecision', () => {
  ...
  it('returns "block" for score >= 0.90', () => {
    const decision = makeDecision(0.90, emptyFactors);
    expect(decision.action).toBe('block');
    expect(decision.thresholdApplied).toBe(0.90);
  ...
  it('includes top risk factor names in EDD reason', () => {
    const factors = makeFactors([
      { name: 'Delegation Exposure', direction: 'increases_risk', contribution: 0.2 },
      { name: 'Account Inactivity', direction: 'increases_risk', contribution: 0.15 },
      { name: 'Credential Count', direction: 'decreases_risk', contribution: -0.05 },
    ]);
    const decision = makeDecision(0.80, factors);
  ...
  it('returns "review" for score in [0.50, 0.75)', () => {
    const decision = makeDecision(0.50, emptyFactors);
    expect(decision.action).toBe('review');
    expect(decision.thresholdApplied).toBe(0.50);
  ...
  it('always provides suggestedActions array', () => {
    for (const score of [0.0, 0.25, 0.50, 0.75, 0.90, 1.0]) {
...
describe('computeRiskTrend', () => {
  ...
  it('returns stable when velocity is within [-0.001, 0.001]', () => {
    const history = [
      { timestamp: 1_000_000, score: 0.500 },
...
describe('predictRisk', () => {
  ...
  it('integrates with historical scores for trend', () => {
    ...
    expect(['improving', 'stable', 'deteriorating']).toContain(prediction.trend.direction);
  });
