...
import { JURISDICTIONS, CROSS_BORDER_MATRIX } from '@/lib/regulatory/jurisdictions';
...
describe('evaluateJurisdiction', () => {
  ...
  it('returns non_compliant with score 0 for unknown jurisdiction', () => {
    const result = evaluateJurisdiction('unknown_place' as JurisdictionId, [], NOW_MS);
  ...
  it('returns compliant (score 100) when all mandatory credentials are active and not expiring', () => {
    ...
    expect(consentMet.length).toBe(JURISDICTIONS.uae.consentRequirements.length);
  });
  ...
  it('returns non_compliant when all mandatory credentials are missing', () => {
    ...
    expect(result.status).toBe('non_compliant');
    expect(result.score).toBe(0);
    const mandatoryCount = JURISDICTIONS.uae.requiredCredentials.filter((r) => r.mandatory).length;
    const mandatoryUnmet = result.unmet.filter((r) => r.mandatory && r.category === 'credential');
  ...
  it('marks credential as unmet when status is not active (e.g. pending)', () => {
    ...
    const govIdUnmet = result.unmet.find((r) => r.credentialSchemaId === 'gov_id');
  ...
  it('marks credential as expiring_soon within 30 days', () => {
    ...
    const expiring = result.expiringSoon.find((r) => r.credentialSchemaId === 'gov_id');
  ...
  it('adds consent requirements as met entries', () => {
    const result = evaluateJurisdiction('eu', makeFullCredentials('eu', NOW_MS), NOW_MS);
    const consentMet = result.met.filter((r) => r.category === 'consent');
  ...
  it('returns score 100 when jurisdiction has zero mandatory credentials', () => {
    ...
    (JURISDICTIONS as Record<string, typeof JURISDICTIONS['uae']>)[testId] = {
      ...
      requiredCredentials: [
        { schemaId: 'optional_cred', schemaName: 'Optional Cred', mandatory: false, validityPeriodDays: 365, acceptedIssuers: [], renewalBufferDays: 30 },
      ],
      consentRequirements: [],
    };
...
describe('calculateRiskScore', () => {
  ...
  it('computes correct weighted scores', () => {
    ...
    for (const factor of result.factors) {
      expect(factor.weightedScore).toBeCloseTo(factor.rawScore * factor.weight, 5);
    }
  });
  ...
  it('credential freshness is 50 when no active credentials exist', () => {
    ...
    const freshness = result.factors.find((f) => f.name === 'credential_freshness');
  ...
  it('applies correct thresholds for risk levels', () => {
    ...
    const lowResult = calculateRiskScore(makeFullCredentials('jp'), 'jp', 10, 500);
  ...
  it('account maturity score varies by age brackets', () => {
    ...
    const youngMaturity = young.factors.find((f) => f.name === 'account_maturity');
    ...
    const olderMaturity = older.factors.find((f) => f.name === 'account_maturity');
    expect(olderMaturity!.rawScore).toBe(15);

    // 365+ days => 5
    const mature = calculateRiskScore([], 'eu', 0, 500);
    const matureMaturity = mature.factors.find((f) => f.name === 'account_maturity');
  ...
  it('transaction velocity score varies by daily tx rate brackets', () => {
    ...
    expect(fast.factors.find((f) => f.name === 'transaction_velocity')!.rawScore).toBe(90);

    // rate > 20, <= 50 => 60
    const moderate = calculateRiskScore([], 'eu', 30, 1);
    expect(moderate.factors.find((f) => f.name === 'transaction_velocity')!.rawScore).toBe(60);

    // rate > 5, <= 20 => 30
    const normal = calculateRiskScore([], 'eu', 10, 1);
    expect(normal.factors.find((f) => f.name === 'transaction_velocity')!.rawScore).toBe(30);

    // rate <= 5 => 10
    const slow = calculateRiskScore([], 'eu', 3, 1);
    expect(slow.factors.find((f) => f.name === 'transaction_velocity')!.rawScore).toBe(10);
  });
});
...
describe('detectCredentialGaps', () => {
  ...
  it('identifies expiring_soon credentials (within 30 days)', () => {
    ...
    const expGap = result.find((g) => g.schemaId === 'gov_id' && g.reason === 'expiring_soon');
  ...
  it('marks expired non-mandatory credential as medium severity', () => {
    ...
    const result = detectCredentialGaps('sa', [...allMandatory, expiredOptional], NOW_MS);
    const absherGap = result.find((g) => g.schemaId === 'absher_verification' && g.reason === 'expired');
  ...
  it('marks expiring_soon non-mandatory credential as low severity', () => {
    ...
    const result = detectCredentialGaps('eu', [...allMandatory, expiringOptional], NOW_MS);
    const residencyGap = result.find((g) => g.schemaId === 'residency' && g.reason === 'expiring_soon');
  ...
  it('marks wrong_issuer non-mandatory credential as medium severity', () => {
    ...
    const result = detectCredentialGaps('sa', [...allMandatory, wrongIssuerOptional], NOW_MS);
    const wrongIssuerGap = result.find((g) => g.schemaId === 'absher_verification' && g.reason === 'wrong_issuer');
    expect(wrongIssuerGap).toBeDefined();
...
describe('computeRegulatoryDeadlines', () => {
  ...
  it('skips credentials not within 90-day expiry window', () => {
    ...
    const expiryDeadlines = result.filter((d) => d.type === 'credential_expiry');
  ...
  it('includes reporting obligation deadlines', () => {
    ...
    expect(reportingDeadlines.length).toBe(JURISDICTIONS.uae.reportingObligations.length);
  });
