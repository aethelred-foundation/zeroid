...
describe("createBlindedCredentialRequest", () => {
  ...
  it("uses provided nonce instead of generating one", async () => {
    ...
    const result = await createBlindedCredentialRequest(
      pk,
      messages,
      [0],
      customNonce,
...
describe("verifyBBSProof", () => {
  ...
  it("validates a well-formed proof", async () => {
    ...
    expect(
      result.checks.find((c) => c.name === "proof_structure")?.passed,
    ).toBe(true);
    expect(result.checks.find((c) => c.name === "domain_binding")?.passed).toBe(
      true,
    );
    expect(result.checks.find((c) => c.name === "index_bounds")?.passed).toBe(
      true,
    );
    expect(
      result.checks.find((c) => c.name === "nonce_freshness")?.passed,
    ).toBe(true);
  });
...
describe("generatePresentation", () => {
  ...
  it("binds multiple proofs to the same domain", async () => {
    ...
    const sig = makeSignature();
    const messages = makeMessages(2);
    const proof1 = await deriveSelectiveDisclosureProof(
      sig,
      pk,
      messages,
      [0],
    ...
    const proof2 = await deriveSelectiveDisclosureProof(
      sig,
      pk,
      messages,
      [1],
...
describe("blindCredential", () => {
  ...
  it("blinds all messages and returns matching blinding factors", async () => {
    ...
    const { blindedMessages, blindingFactors } =
      await blindCredential(messages);
...
describe("unblindCredential", () => {
  ...
  it("throws when blinded messages and factors have different lengths", async () => {
    await expect(unblindCredential(["0xa", "0xb"], ["0xf1"])).rejects.toThrow(
      "Blinded messages and factors length mismatch",
    );
  });
...
describe("verifyMultiMessageCommitment", () => {
  ...
  it("returns true for a correctly opened commitment", async () => {
    ...
    digestCallCount = 0;
    const isValid = await verifyMultiMessageCommitment(
      commitment,
      messages,
      generators,
    );
