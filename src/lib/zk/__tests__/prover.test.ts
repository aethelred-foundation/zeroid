...
describe("prover", () => {
  ...
  describe("generateProof", () => {
    ...
    it("throws when all inputs are missing", async () => {
      await expect(generateProof(circuitId, {}, {})).rejects.toThrow(
        /Missing circuit inputs/,
      );
    });
    ...
    it("calls the onProgress callback at each stage", async () => {
      ...
      expect(onProgress).toHaveBeenCalledWith(
        15,
        "Fetching WASM proving circuit",
      );
      expect(onProgress).toHaveBeenCalledWith(40, "Preparing witness inputs");
      expect(onProgress).toHaveBeenCalledWith(
        50,
        expect.stringContaining("Generating ZK proof"),
      );
