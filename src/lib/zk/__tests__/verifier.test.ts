...
const MOCK_VKEY = {
  ...
  vk_alpha_1: ['1', '2', '3'],
  vk_beta_2: [['4', '5'], ['6', '7']],
  vk_gamma_2: [['8', '9'], ['10', '11']],
  vk_delta_2: [['12', '13'], ['14', '15']],
...
function makeZKProof(overrides: Partial<ZKProof> = {}): ZKProof {
  ...
  return {
    ...
    proofHash: '0xdeadbeef00000000000000000000000000000000000000000000000000000000' as Bytes32,
    ...overrides,
  };
}
...
describe('verifier', () => {
  ...
  describe('verifyRawProof', () => {
    ...
    it('throws when fetch for vkey fails', async () => {
      ...
      await expect(
        verifyRawProof(circuitId, proof, signals),
    ...
    it('propagates snarkjs errors', async () => {
      ...
      await expect(
        verifyRawProof(circuitId, proof, signals),
      ).rejects.toThrow('BN128 pairing failed');
    });
  });
  ...
  describe('verification key caching', () => {
    ...
    it('caches per circuit ID', async () => {
      ...
      await verifyProofLocally(makeZKProof({ circuitId: '0xage01' as Bytes32 }));
      expect(mockFetch).toHaveBeenCalledTimes(1);
