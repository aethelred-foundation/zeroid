...
/**
 * Generate an RFC 4122 v4 UUID in any browsing context.
 *
 * `crypto.randomUUID` is only exposed in secure contexts (HTTPS or localhost),
 * so on a plain-HTTP remote origin — e.g. a testnet box reached by IP — it is
 * undefined and calling it crashes every code path that needs an id. Fall back
 * to `crypto.getRandomValues`, which is available in ALL contexts and equally
 * cryptographically random (no weak Math.random fallback: if crypto is absent
 * entirely, this throws, which is the correct failure).
 */
