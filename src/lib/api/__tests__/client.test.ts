...
const mockWithTimeout = jest.fn(
  async <T>(promise: Promise<T>, _ms: number, _msg?: string): Promise<T> => promise,
);

jest.mock('@/lib/utils', () => ({
  withRetry: (...args: unknown[]) => mockWithRetry(...(args as [() => Promise<unknown>, number])),
...
describe('ZeroIDApiError', () => {
  ...
  it('stores all constructor properties', () => {
    ...
    const err = new ZeroIDApiError('bad request', 'VALIDATION', 422, details, 'zid-abc-def');
...
describe('request internals (tested via apiClient methods)', () => {
  ...
  it('throws ZeroIDApiError on non-ok response with error body', async () => {
    mockFetch.mockResolvedValue(
      errorResponse('NOT_FOUND', 'Identity not found', 404, 'Not Found', { didHash: '0x00' }),
    );
    await expect(apiClient.health()).rejects.toThrow(ZeroIDApiError);
    try {
      await apiClient.health();
  ...
  it('serialises body as JSON on POST requests', async () => {
    mockFetch.mockResolvedValue(jsonResponse({ didHash: '0xabc', txHash: '0xdef' }));
    const payload = { didUri: 'did:aethelred:mainnet:0x1', recoveryHash: '0xrecov' };
...
describe('retry behaviour', () => {
  ...
  it('retries GET requests on failure', async () => {
    let attempt = 0;
    mockWithRetry.mockImplementation(async (fn: () => Promise<unknown>, retries: number) => {
      let lastErr: unknown;
...
describe('timeout behaviour', () => {
  ...
  it('rejects with timeout error when withTimeout rejects', async () => {
    mockWithTimeout.mockRejectedValueOnce(new Error('ZeroID API request timed out after 30000ms (GET /api/v1/health)'));
...
describe('apiClient.health()', () => {
  it('calls GET /api/v1/health and returns data', async () => {
    mockFetch.mockResolvedValue(jsonResponse({ status: 'healthy', version: '1.0' }));
    const result = await apiClient.health();
...
describe('apiClient.getIdentityByAddress()', () => {
  it('calls GET /api/v1/identity/address/{address}', async () => {
    ...
    const result = await apiClient.getIdentityByAddress('0xAddr' as `0x${string}`);
    expect(result).toBeNull();
    expect(mockFetch.mock.calls[0][0]).toContain('/api/v1/identity/address/0xAddr');
  });
});
...
describe('apiClient.registerIdentity()', () => {
  it('calls POST /api/v1/identity/register with payload', async () => {
    ...
    const payload = { didUri: 'did:aethelred:mainnet:0x1', recoveryHash: '0xrecov' as `0x${string}` };
...
describe('apiClient.getVerificationResult()', () => {
  it('calls GET /api/v1/proofs/verifications/{id}', async () => {
    ...
    expect(mockFetch.mock.calls[0][0]).toContain('/api/v1/proofs/verifications/req-123');
  });
});
...
describe('apiClient.getAttestation()', () => {
  it('calls GET /api/v1/tee/attestation/{hash}', async () => {
    ...
    expect(mockFetch.mock.calls[0][0]).toContain('/api/v1/tee/attestation/0xenc');
  });
});
...
describe('apiClient.requestBiometricVerification()', () => {
  it('calls POST /api/v1/tee/biometric/verify', async () => {
    ...
    const result = await apiClient.requestBiometricVerification(payload, 'auth');
...
describe('apiClient.createVerificationRequest()', () => {
  it('calls POST /api/v1/verifications', async () => {
    const payload = { verifierDid: '0xv', subjectDid: '0xs', schemaHash: '0xsch' };
    mockFetch.mockResolvedValue(jsonResponse({ id: 'vr1', status: 'pending' }));
    const result = await apiClient.createVerificationRequest(payload as any, 'auth');
...
describe('apiClient.respondToVerification()', () => {
  it('calls POST /api/v1/verifications/{id}/respond', async () => {
    ...
    mockFetch.mockResolvedValue(jsonResponse({ status: 'verified' }));
    const result = await apiClient.respondToVerification('req-1', payload as any, 'auth');
...
describe('apiClient.getProposal()', () => {
  it('calls GET /api/v1/governance/proposals/{id}', async () => {
    ...
    expect(mockFetch.mock.calls[0][0]).toContain('/api/v1/governance/proposals/42');
  });
});
