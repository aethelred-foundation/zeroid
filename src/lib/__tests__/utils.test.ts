...
jest.mock('clsx', () => ({
  clsx: jest.fn((...args: unknown[]) => args.flat(Infinity).filter(Boolean).join(' ')),
}));
...
describe('formatDateTime', () => {
  it('calls formatDate with the date-time pattern', () => {
    ...
    expect(format).toHaveBeenCalledWith(expect.any(Date), "MMM d, yyyy 'at' h:mm a");
  });
});
...
describe('createDID', () => {
  ...
  it('creates a DID with default mainnet network', () => {
    ...
    expect(toBytes).toHaveBeenCalledWith('did:aethelred:mainnet:0xidentifier123');
