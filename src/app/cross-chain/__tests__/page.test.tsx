...
describe('CrossChainPage', () => {
  ...
  it('shows metric cards', () => {
    render(<CrossChainPage />);
    expect(screen.getAllByText('Supported Chains').length).toBeGreaterThanOrEqual(1);
  ...
  it('switches tabs when clicking on tab buttons', () => {
    ...
    const bridgedCredTab = tabButtons.find(btn => btn.textContent?.includes('Bridged Credentials'));
    fireEvent.click(bridgedCredTab!);
    expect(screen.getByText('KYC Identity Verification')).toBeInTheDocument();

    // Click on 'History' tab
    fireEvent.click(screen.getByText('History'));
    expect(screen.getByText('Cross-Chain Verification History')).toBeInTheDocument();
  });
  ...
  it('has a swap button that can be clicked without error', () => {
    ...
    const swapButton = allButtons.find(btn => !btn.textContent || btn.textContent.trim() === '');
