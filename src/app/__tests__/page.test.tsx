...
jest.mock('@/hooks/useCredentials', () => ({
  useCredentials: jest.fn(() => ({
    data: { credentials: [{ id: '1', status: 'active' }, { id: '2', status: 'active' }] },
    isLoading: false,
  })),
}));
...
jest.mock('@/components/ui/MetricCard', () => ({
  MetricCard: ({ label, value }: any) => <div data-testid="metric-card">{label}: {value}</div>,
}));
...
describe('DashboardPage', () => {
  ...
  it('handles null credentials array', () => {
    (useAccount as jest.Mock).mockReturnValue({ address: '0x1234', isConnected: true });
    const { useCredentials } = require('@/hooks/useCredentials');
    useCredentials.mockReturnValue({ data: { credentials: null }, isLoading: false });
    render(<DashboardPage />);
    expect(screen.getByTestId('app-layout')).toBeInTheDocument();
  });
  ...
  it('handles mixed credential statuses', () => {
    ...
    useCredentials.mockReturnValue({
      data: { credentials: [{ id: '1', status: 'active' }, { id: '2', status: 'expired' }, { id: '3', status: 'revoked' }] },
      isLoading: false,
    });
    render(<DashboardPage />);
  ...
  it('handles null verificationHistory', () => {
    ...
    useVerification.mockReturnValue({ verificationHistory: null, isLoading: false });
    render(<DashboardPage />);
    expect(screen.getByTestId('app-layout')).toBeInTheDocument();
  });
