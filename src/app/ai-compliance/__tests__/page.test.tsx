...
describe('AICompliancePage', () => {
  ...
  it('displays the page heading', () => {
    render(<AICompliancePage />);
    expect(screen.getByText('AI Compliance Command Center')).toBeInTheDocument();
  });
  ...
  it('shows risk feed by default', () => {
    ...
    expect(screen.getAllByText(/OFAC SDN list updated/).length).toBeGreaterThan(0);
  });

  it('switches to screening tab', () => {
    render(<AICompliancePage />);
    const screeningTab = screen.getByRole('button', { name: /Sanctions Screening/i });
    fireEvent.click(screeningTab);
    expect(screen.getByPlaceholderText(/Search names, entities/)).toBeInTheDocument();
  });
  ...
  it('switches to heatmap tab and renders jurisdiction data', () => {
    ...
    expect(screen.getByText('Compliance Risk by Jurisdiction')).toBeInTheDocument();
  ...
  it('can switch back to feed tab from another tab', () => {
    render(<AICompliancePage />);
    fireEvent.click(screen.getByRole('button', { name: /Sanctions Screening/i }));
    expect(screen.queryByText('Live Risk Intelligence Feed')).not.toBeInTheDocument();
  ...
  it('expands and collapses PEP match details', () => {
    ...
    expect(screen.queryByText(/Direct PEP. Held government position/)).not.toBeInTheDocument();

    // Click to expand
    fireEvent.click(screen.getByText('Ahmed Al-Fahim'));
    expect(screen.getByText(/Direct PEP. Held government position/)).toBeInTheDocument();
  ...
  it('expands a different PEP when another is already expanded', () => {
    ...
    expect(screen.getByText(/Current PEP. Active political figure/)).toBeInTheDocument();
  ...
  it('renders AI Copilot chat section with initial messages', () => {
    ...
    expect(screen.getByText(/completed the overnight compliance scan/)).toBeInTheDocument();
  });
  ...
  it('sends a chat message via Enter key and receives a response', () => {
    ...
    expect(screen.getAllByText(/I've analyzed the request/).length).toBeGreaterThanOrEqual(1);
  });
