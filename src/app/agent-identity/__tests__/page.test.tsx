...
describe('AgentIdentityPage', () => {
  ...
  it('displays the page heading', () => {
    render(<AgentIdentityPage />);
    expect(screen.getByText('AI Agent Identity Management')).toBeInTheDocument();
  });
  ...
  it('renders agent cards in grid view by default', () => {
    render(<AgentIdentityPage />);
    expect(screen.getAllByText('ComplianceBot-Alpha').length).toBeGreaterThan(0);
  ...
  it('filters agents by search query', () => {
    render(<AgentIdentityPage />);
    const searchInput = screen.getByPlaceholderText('Search agents by name or type...');
  ...
  it('navigates through wizard steps', () => {
    ...
    expect(screen.getAllByText('Register Agent').length).toBeGreaterThanOrEqual(2);
  });
  ...
  it('switches to Delegation Chains tab', () => {
    ...
    expect(screen.getByText('Delegation Chain Visualization')).toBeInTheDocument();
  ...
  it('displays HITL approval queue', () => {
    ...
    expect(screen.getByText('Execute swap: 50,000 USDC -> ETH')).toBeInTheDocument();
  });

  it('displays real-time activity log', () => {
    render(<AgentIdentityPage />);
    expect(screen.getByText('Real-time Activity')).toBeInTheDocument();
    expect(screen.getByText('Completed sanctions screening batch (247 entities)')).toBeInTheDocument();
  });
  ...
  it('selects an agent card in grid view', () => {
    ...
    fireEvent.click(agentCards[0].closest('[class*="cursor-pointer"]') || agentCards[0]);
  ...
  it('selects an agent in list view', () => {
    ...
    const viewToggle = screen.getByPlaceholderText('Search agents by name or type...').parentElement?.parentElement;
    ...
    const clickTarget = agentName.closest('[class*="cursor-pointer"]') || agentName;
