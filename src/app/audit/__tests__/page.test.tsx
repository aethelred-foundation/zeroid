...
jest.mock('@/hooks/useAudit', () => ({
  useAudit: jest.fn(() => ({
    auditLog: [
      { id: '1', action: 'credential_issued', category: 'credentials', timestamp: new Date().toISOString() },
      { id: '2', action: 'proof_generated', category: 'verifications', timestamp: new Date().toISOString() },
      { id: '3', action: 'credential_revoked', category: 'credentials', timestamp: new Date().toISOString() },
    ],
    isLoading: false,
  })),
}));
...
describe('AuditPage', () => {
  ...
  it('displays the page heading', () => {
    ...
    expect(screen.getByText(/Complete history of identity actions/)).toBeInTheDocument();
  });
  ...
  it('switches to different category tabs', () => {
    ...
    expect(screen.getByRole('button', { name: /Verifications/i })).toBeInTheDocument();
  ...
  it('handles null auditLog gracefully', () => {
    (useAudit as jest.Mock).mockReturnValue({ auditLog: null, isLoading: false });
    render(<AuditPage />);
    expect(screen.getByTestId('app-layout')).toBeInTheDocument();
    expect(screen.getByText('Total Events')).toBeInTheDocument();
  });
  ...
  it('handles undefined auditLog gracefully', () => {
    (useAudit as jest.Mock).mockReturnValue({ auditLog: undefined, isLoading: false });
    render(<AuditPage />);
    expect(screen.getByTestId('app-layout')).toBeInTheDocument();
  });
