...
jest.mock('@/components/verification/SelectiveDisclosureBuilder', () => ({
  __esModule: true,
  default: () => <div data-testid="selective-disclosure">Selective Disclosure Builder</div>,
}));

jest.mock('@/components/zkp/ProofVisualization', () => ({
  __esModule: true,
  default: ({ proofId, onClose }: any) => (
    <div data-testid="proof-visualization">Proof: {proofId} <button onClick={onClose}>Close</button></div>
  ),
}));
...
const defaultVerificationData = {
  verificationHistory: [
    { id: 'v1', proofType: 'Age', verifier: 'Cruzible', status: 'verified', timestamp: '2025-03-01' },
    { id: 'v2', proofType: 'KYC', verifier: 'NoblePay', status: 'pending', timestamp: '2025-03-02' },
  ],
  pendingRequests: [
...
describe('VerificationPage', () => {
  ...
  it('shows empty state when verification history is empty', () => {
    ...
    expect(screen.getByText('Generate your first ZK proof to get started')).toBeInTheDocument();
  });
  ...
  it('renders revoked status styling for history items', () => {
    mockUseVerification.mockReturnValue({
      verificationHistory: [
        { id: 'v1', proofType: 'Age', verifier: 'Cruzible', status: 'verified', timestamp: '2025-03-01' },
        { id: 'v2', proofType: 'KYC', verifier: 'NoblePay', status: 'pending', timestamp: '2025-03-02' },
        { id: 'v3', proofType: 'AML', verifier: 'Exchange', status: 'revoked', timestamp: '2025-03-03' },
      ],
      pendingRequests: [],
    });
    render(<VerificationPage />);
