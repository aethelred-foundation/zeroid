...
const mockUseGovernance = jest.fn(() => ({
  proposals: [
    { id: '1', title: 'Add KYC schema', status: 'active', votesFor: 100, votesAgainst: 20 },
    { id: '2', title: 'Update fee structure', status: 'passed', votesFor: 200, votesAgainst: 10 },
    { id: '3', title: 'Remove old issuer', status: 'rejected', votesFor: 30, votesAgainst: 150 },
  ],
  votingPower: 5000,
  isLoading: false,
}));
...
jest.mock('@/components/governance/VotingPanel', () => ({
  ...
  default: ({ proposal, onVoteSubmitted }: any) => (
    <div data-testid="voting-panel">
      {proposal.title}
      <button data-testid="submit-vote" onClick={onVoteSubmitted}>Submit Vote</button>
    </div>
  ),
}));
...
describe('GovernancePage', () => {
  ...
  it('shows empty state for all filter when proposals is empty', () => {
    ...
    render(<GovernancePage />);
    expect(screen.getByText('No proposals found')).toBeInTheDocument();
    expect(screen.getByText('Be the first to create a governance proposal')).toBeInTheDocument();
  });
  ...
  it('computes pass rate as 0 when proposals array is empty', () => {
    ...
    expect(screen.getByTestId('metric-Your Voting Power')).toHaveTextContent('0');
  });
  ...
  it('handles null proposals gracefully', () => {
    ...
    render(<GovernancePage />);
    expect(screen.getByText('No proposals found')).toBeInTheDocument();
    expect(screen.getByTestId('metric-Active Proposals')).toHaveTextContent('0');
  });
  ...
  it('exercises proposals nullish coalescing in VotingPanel branch via dynamic proposals', () => {
    ...
    let callCount = 0;
    const proposal1 = { id: '1', title: 'Add KYC schema', status: 'active', votesFor: 100, votesAgainst: 20 };
