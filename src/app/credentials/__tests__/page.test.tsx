...
jest.mock("@/components/credentials/CredentialCard", () => ({
  ...
  default: ({ credential }: any) => (
    <div data-testid="credential-card">{credential.schemaType}</div>
  ),
}));
...
describe("CredentialsPage", () => {
  ...
  it("displays the page heading", () => {
    ...
    expect(
      screen.getByText(/Manage your verifiable credentials/),
    ).toBeInTheDocument();
  });
  ...
  it("shows available credential schemas", () => {
    render(<CredentialsPage />);
    expect(
      screen.getByText("Available Credential Schemas"),
    ).toBeInTheDocument();
  ...
  it("shows empty state with Request Credential button when no credentials and filter is all", () => {
    ...
    expect(screen.getByText("No credentials found")).toBeInTheDocument();
    expect(
      screen.getByText("Request your first credential to get started"),
    ).toBeInTheDocument();
  });
  ...
  it("switches back to grid view from list view", () => {
    ...
    const viewButtons = allButtons2.filter((btn) =>
      (btn.getAttribute("class") || "").includes("p-2.5"),
    );
