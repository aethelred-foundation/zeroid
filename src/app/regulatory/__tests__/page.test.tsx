...
describe('RegulatoryPage', () => {
  ...
  it('displays the page heading', () => {
    render(<RegulatoryPage />);
    expect(screen.getByText('Multi-Jurisdiction Regulatory Dashboard')).toBeInTheDocument();
  });
  ...
  it('shows jurisdiction map by default', () => {
    render(<RegulatoryPage />);
    expect(screen.getByText('Compliance Status by Jurisdiction')).toBeInTheDocument();
    expect(screen.getAllByText('United States').length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText('United Arab Emirates').length).toBeGreaterThanOrEqual(1);
  });
  ...
  it('shows regulatory change feed in sidebar', () => {
    ...
    expect(screen.getByText('MiCA enters full enforcement')).toBeInTheDocument();
  });
  ...
  it('switches to Cross-Border Checker tab and shows routes', () => {
    ...
    expect(screen.getByText('Cross-Border Transfer Compliance Checker')).toBeInTheDocument();
    // Check cross-border routes are rendered
    expect(screen.getByText('Bilateral MOU active. Standard KYC sufficient.')).toBeInTheDocument();
  });
  ...
  it('shows privacy framework compliance indicators', () => {
    render(<RegulatoryPage />);
    expect(screen.getByText('Privacy Framework Compliance')).toBeInTheDocument();
  ...
  it('deselects a jurisdiction when clicking the same one again', () => {
    ...
    const usBtn = usElements.find(el => el.closest('button'))!.closest('button')!;
    // Select
    fireEvent.click(usBtn);
    expect(screen.getByText('94/100')).toBeInTheDocument();
    // Deselect by clicking same button again (re-query)
    const usElements2 = screen.getAllByText('United States');
    const usBtn2 = usElements2.find(el => el.closest('button'))!.closest('button')!;
    fireEvent.click(usBtn2);
    expect(screen.getByText('Select a jurisdiction to view details')).toBeInTheDocument();
  });
